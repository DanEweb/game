...
import { FX } from "./fx.js";
  ...
  wrap.addEventListener('pointerup', (e)=>{
    ...
    const sx0 = tapInfo.x, sy0 = tapInfo.y;
    tapInfo = null;
    if (state!=='playing' || !player) return;
    // v6.68 드래그 밀치기 인터랙션: [밀쳐내기] 기믹 중, 보스 방향으로 힘껏 드래그하면 밀쳐낸다
    if (player.gimShoveT>0 && dp>60 && held<650){
      const bbS = player.gimShoveBoss;
      if (bbS && bosses.includes(bbS)){
        const dragA = Math.atan2(e.clientY-sy0, e.clientX-sx0);
        const toB = Math.atan2(bbS.y-player.y, bbS.x-player.x);
        let dA = dragA - toB;
        while (dA>Math.PI) dA -= Math.PI*2;
        while (dA<-Math.PI) dA += Math.PI*2;
        if (Math.abs(dA) < 0.7){
          player.gimShoveT = 0; player.gimShoveBoss = null;
          bbS.x += Math.cos(toB)*150; bbS.y += Math.sin(toB)*150;
          bbS.hp -= bbS.maxHp*0.05;
          addDmgNum(bbS.x, bbS.y, bbS.maxHp*0.05, true);
          addTextNum(bbS.x, bbS.y-bbS.r-16, '밀쳐냈다!');
          effects.push({ type:'ring', x:bbS.x, y:bbS.y, life:0.4, age:0, r0:bbS.r*0.5, r1:bbS.r*2, col:'#e2823f' });
          refreshBossBar();
          if (bbS.hp<=0){ const bi=bosses.indexOf(bbS); if (bi>=0) defeatBoss(bi); }
          SFX.play('boom');
          return;
        }
      }
    }
    if (dp>9 || held>320) return;
    ...
    for (let i=gateObjs.length-1;i>=0;i--){
      const o = gateObjs[i];
      // v6.68 리듬 인터랙션: 수축하는 링이 코어에 닿는 순간에 맞춰 탭 — 이르면 실패
      if (o.kind==='rhythm' && (o.x-wx)**2+(o.y-wy)**2 < (o.r+40)**2){
        gateObjs.splice(i,1);
        if (o.t <= (o.window||0.45)){ if (o.onTap) o.onTap(); SFX.play('quest'); }
        else { addTextNum(o.x, o.y-o.r-8, '너무 이르다!'); if (o.onFail) o.onFail(); SFX.play('warn'); }
        return;
      }
      // v6.68 진짜 찾기(find) 인터랙션: 미세하게 빛나는 진짜를 간파하라 — 허상을 고르면 반격
      if (o.kind==='find' && (o.x-wx)**2+(o.y-wy)**2 < (o.r+34)**2){
        const wasReal = !!o.real;
        const cbT = o.onTap, cbW = o.onWrong;
        for (let k2=gateObjs.length-1;k2>=0;k2--){
          if (gateObjs[k2].kind==='find' && gateObjs[k2].grp===o.grp) gateObjs.splice(k2,1);
        }
        if (wasReal){ if (cbT) cbT(); SFX.play('quest'); }
        else { if (cbW) cbW(); SFX.play('warn'); }
        return;
      }
      // v6.67 순서(seq) 인터랙션: ①→②→③ 순서대로 탭 — 틀리면 봉인이 뒤틀린다
      if (o.kind==='seq' && (o.x-wx)**2+(o.y-wy)**2 < (o.r+34)**2){
        const grpObjs = gateObjs.filter(g2=>g2.kind==='seq' && g2.grp===o.grp);
        const minSeq = Math.min.apply(null, grpObjs.map(g2=>g2.seq));
        if (o.seq===minSeq){
          gateObjs.splice(i,1);
          burst(o.x, o.y, 3, 100);
          SFX.play('hit');
          if (grpObjs.length===1 && o.onTap) o.onTap(); // 마지막 인장 = 성공
        } else {
          for (let k2=gateObjs.length-1;k2>=0;k2--){
            if (gateObjs[k2].kind==='seq' && gateObjs[k2].grp===o.grp) gateObjs.splice(k2,1);
          }
      ...
      if ((o.kind==='qte' || o.kind==='mash') && (o.x-wx)**2+(o.y-wy)**2 < (o.r+34)**2){
        // v6.67 연타(mash) 인터랙션: 여러 번 탭해야 풀린다
        if (o.kind==='mash'){
          o.need = (o.need||5) - 1;
          burst(o.x, o.y, 2, 90);
          addTextNum(o.x, o.y-o.r-8, o.need>0 ? o.need+'번 더!' : '성공!');
          SFX.play('hit');
          if (o.need>0) return;
        }
        gateObjs.splice(i,1);
  ...
  function renderEquip(){
    ...
    (CGW_NAMES[equipClassTab]||[]).forEach((nm, i)=>{
      ...
      GW_LIST.push({ key:'cgw_'+equipClassTab+'_'+i, found: !!(rec&&rec.found), lv: rec?rec.lv:1,
        ...
        ds:'[직업 유일] 극악 확률 발견 · 장착 후 보스 처치로 성장' });
    });
    // 직업 성장무기 3종 (공방 제작): 보유 시 장착 목록에 합류
    // 성능이 동일하므로 제작비도 동일 — 셋의 차이는 오직 '손맛'(원형)
    const PGW_COST = [
      { shard:2, essence:1, gear:1, gold:700 },
      { shard:1, essence:2, gear:1, gold:700 },
      { shard:1, essence:1, gear:2, gold:700 },
    ];
    for (let pi=0; pi<3; pi++){
      const pkey = 'pgw_'+equipClassTab+'_'+pi;
      if (!WEAPONS[pkey]) break;
      ...
      if (prec && prec.found){
        ...
        const needAwk = {10:1,20:2,30:3,40:4}[prec.lv];
        ...
        const row = document.createElement('div');
        row.className = 'shopItem';
        row.innerHTML = '<div class="info"><div class="nm">🔨 '+WEAPONS[pkey].name+' <span style="font-size:9px;color:var(--ink-500);">[미제작]</span></div>'
          + '<div class="ds">공방 제작 — ★'+c.shard+' ◆'+c.essence+' ⚙'+c.gear+' + '+c.gold+'G</div></div>';
        const mk = document.createElement('button');
        mk.className = 'buy';
        const can = DB.mats.shard>=c.shard && DB.mats.essence>=c.essence && DB.mats.gear>=c.gear && DB.gold>=c.gold;
        mk.textContent = '제작';
        mk.disabled = !can;
  ...
  function renderShop(){
    ...
    sep.className = 'shopItem';
    sep.style.cssText = 'background:rgba(184,54,46,0.08); border:1px dashed rgba(184,54,46,0.4);';
    sep.innerHTML = '<div class="info"><div class="nm">🎰 뒷골목 도박장</div><div class="ds">기대값은 언제나 하우스 편 — 그래도 들어올 거잖아?</div></div>';
    ...
    [
      ...
      const row = document.createElement('div');
      row.className = 'shopItem';
      row.innerHTML = '<div class="info"><div class="nm">'+c.nm+'</div><div class="ds">'+c.ds+'</div></div>';
      const buy = document.createElement('button');
      buy.className = 'buy';
      buy.textContent = c.cost+'G';
      buy.disabled = DB.gold < c.cost;
  ...
  const WEAPONS = {
    ...
    iaido: {
      name:'발도', desc:'짧게 뽑아 베고 곧장 납도한다 — 맞지 않은 채 벨수록 잔심이 배로 찬다. 만충 시 R로 화면을 가르는 거합',
      evName:'발도 · 무명일섬', evDesc:'참격이 화면을 가르고, 벤 자리에 검흔이 남습니다',
      lvDesc:['','참격 연장','피해 +40%','참격 범위 확대','피해 강화'],
      // v6.114 평타는 근접 부채꼴로 되돌린다 — 직선 장참(거합)은 R 소비기 전용이 되었다
      baseCd:(w)=> w.evolved ? 0.52 : 0.62,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 16 : [6,6,8,8,11][w.lv-1]),
      radius:(w)=> (w.evolved ? 118 : [84,92,92,102,102][w.lv-1]),
      arc:(w)=> (w.evolved ? 2.5 : [1.9,1.9,2.1,2.1,2.3][w.lv-1])
    },
    ...
    kesagiri: {
      name:'연참', desc:'가볍고 빠르게 벤다 — 서로 다른 적을 벨수록 표식이 빨리 찬다. 만충 시 R로 적 사이를 순간이동하며 난무',
      evName:'연참 · 무영난무', evDesc:'표식 요구가 줄고 연참 피해가 크게 오릅니다',
      lvDesc:['','표식 요구 -1','피해 +40%','연참 피해 +50%','피해 강화'],
      baseCd:(w)=> w.evolved ? 0.38 : 0.44,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 11 : [4,4,6,6,8][w.lv-1]),
      radius:(w)=> (w.evolved ? 96 : [66,72,72,80,80][w.lv-1]),
      arc:(w)=> (w.evolved ? 2.2 : 1.8),
      need:(w)=> (w.evolved ? 2 : 3),
      burst:(w)=> (w.evolved ? 3.4 : [2.2,2.2,2.2,2.8,2.8][w.lv-1])  // 무영난무 1타 배율
    },
    // v6.103 근접 엔진 4호: 회전 참격 — 자기중심 면을 소유한다. 잃은 체력이 곧 위력(응혈)
    whirl: {
      name:'회전 참격', desc:'몸을 돌려 사방을 훑는다 — 피를 흘릴수록 응혈이 빨리 찬다. 만충 시 R로 2초간 멈추지 않는 혈풍륜',
      evName:'회전 참격 · 혈풍륜', evDesc:'회전이 멈추지 않고, 벤 자리에 핏빛 잔풍이 남습니다',
      lvDesc:['','회전 범위 확대','피해 +40%','회전 주기 단축','피해 강화'],
      baseCd:(w)=> w.evolved ? 0.44 : 0.52,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 13 : [5,5,7,7,9][w.lv-1]),
      radius:(w)=> (w.evolved ? 112 : [72,82,82,92,92][w.lv-1]),
      arc:(w)=> Math.PI*2
    },
    // v6.104 엔진 5호: 연격 — 소유물은 '연쇄'. 끊기지 않고 이어질수록 강해진다(연 자원 내장)
    combo3: {
      name:'연격', desc:'전방을 아주 빠르게 벤다 — 끊기지 않고 이어갈수록 연이 빨리 찬다. 만충 시 R로 백팔연타',
      evName:'연격 · 백팔연타', evDesc:'연쇄 상한이 늘고 마무리가 폭발합니다',
      lvDesc:['','연쇄 상한 +2','피해 +40%','범위 확대','피해 강화'],
      baseCd:(w)=> w.evolved ? 0.32 : 0.38,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 10 : [4,4,5,5,7][w.lv-1]),
      radius:(w)=> (w.evolved ? 80 : [56,60,60,68,68][w.lv-1]),
      arc:(w)=> (w.evolved ? 2.1 : 1.7),
      cap:(w)=> (w.evolved ? 8 : [5,7,7,7,7][w.lv-1])
    },
    // v6.104 엔진 6호: 후려치기 — 소유물은 '한 방'. 최대체력이 곧 위력(벌크업 자원 내장)
    smash: {
      name:'후려치기', desc:'묵직하게 후려친다 — 최대체력이 높을수록 벌크가 빨리 찬다. 만충 시 R로 땅을 흔드는 지진',
      evName:'후려치기 · 삼대 오백', evDesc:'충격이 주변으로 퍼지고 넉백이 커집니다',
      lvDesc:['','사거리 확대','피해 +45%','충격 확산','피해 강화'],
      baseCd:(w)=> w.evolved ? 0.60 : 0.70,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 17 : [7,7,9,9,12][w.lv-1]),
      radius:(w)=> (w.evolved ? 96 : [64,74,74,82,82][w.lv-1]),
      arc:(w)=> (w.evolved ? 1.8 : 1.4),
      reach:(w)=> (w.evolved ? 96 : [64,74,74,82,82][w.lv-1]),
      splash:(w)=> (w.evolved ? 62 : [0,0,0,44,44][w.lv-1])
    },
    // v6.105 엔진 7호: 반격 태세 — 소유물은 '정지'. 멈춰 있는 시간이 곧 위력(태세 자원 내장)
    riposte: {
      name:'반격 태세', desc:'멈춰 선 채로 벨수록 태세가 배로 찬다 — 만충 시 R로 1.4초 무적, 그 사이 받은 것을 되돌려준다',
      evName:'반격 태세 · 요지부동', evDesc:'태세 상한이 오르고 반격이 주변 전체로 퍼집니다',
      lvDesc:['','태세 상한 +1','피해 +40%','반격 범위 확대','피해 강화'],
      baseCd:(w)=> w.evolved ? 0.47 : 0.55,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 14 : [5,5,7,7,10][w.lv-1]),
      radius:(w)=> (w.evolved ? 104 : [70,78,78,86,86][w.lv-1]),
      arc:(w)=> (w.evolved ? 2.4 : 1.9),
      cap:(w)=> (w.evolved ? 5 : [3,4,4,4,4][w.lv-1])
    },
    // v6.105 엔진 8호: 광란 난도질 — 소유물은 '가속'. 때릴수록 빨라지고 제 피도 깎인다(광기 자원 내장)
    frenzy: {
      name:'광란 난도질', desc:'짧게 마구 베어댄다 — 제 피를 깎아가며 광기가 빨리 찬다. 만충 시 R로 4초간 자해 없는 광시곡',
      evName:'광란 난도질 · 피의 광시곡', evDesc:'가속 상한이 크게 오르고 자해가 줄어듭니다',
      lvDesc:['','가속 상한 +','피해 +40%','범위 확대','피해 강화'],
      baseCd:(w)=> w.evolved ? 0.29 : 0.34,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 8 : [3,3,4,4,6][w.lv-1]),
      radius:(w)=> (w.evolved ? 84 : [58,62,62,70,70][w.lv-1]),
      arc:(w)=> (w.evolved ? 2.2 : 1.8),
      rush:(w)=> (w.evolved ? 2.4 : 1.9)                       // 광시곡 가속 상한
    },
    // v6.106 엔진 9호: 대낫 처형 — 소유물은 '문턱'. 약해진 적일수록 급격히 아프다
    reap: {
      name:'대낫 처형', desc:'낫을 끌어 벤다 — 약해진 적을 벨수록 수확이 크게 찬다. 만충 시 R로 문턱 아래를 전부 그 자리에서 거둔다',
      evName:'대낫 처형 · 수확의 밤', evDesc:'문턱이 오르고 거둘 때마다 낫이 잠시 넓어집니다',
      lvDesc:['','문턱 +4%p','피해 +40%','범위 확대','피해 강화'],
      baseCd:(w)=> w.evolved ? 0.51 : 0.60,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 15 : [6,6,8,8,10][w.lv-1]),
      radius:(w)=> (w.evolved ? 118 : [78,86,86,96,96][w.lv-1]),
      arc:(w)=> (w.evolved ? Math.PI*2 : 2.1),
      thr:(w)=> (w.evolved ? 0.55 : [0.34,0.40,0.40,0.46,0.46][w.lv-1])   // 수확의 밤 처형 문턱
    },
    // v6.106 엔진 10호: 박자 타격 — 소유물은 '박'. 박에 맞춰 치면 강해진다
    cadence: {
      name:'박자 타격', desc:'박자에 맞춰 벤다 — 정박에 얹어 벨수록 박이 배로 찬다. 만충 시 R로 3초간 모든 타격이 정박',
      evName:'박자 타격 · 광시곡', evDesc:'박이 더 빨라지고 정박 보너스가 커집니다',
      lvDesc:['','박 가속','피해 +40%','범위 확대','피해 강화'],
      baseCd:(w)=> w.evolved ? 0.39 : 0.46,
      dmg:(w)=> meleeAutoMult()*(w.evolved ? 11 : [4,4,6,6,8][w.lv-1]),
      radius:(w)=> (w.evolved ? 90 : [62,68,68,76,76][w.lv-1]),
      arc:(w)=> (w.evolved ? 2.3 : 1.9)
    },
  ...
  const TREES = {
    ...
    fire: { name:'지옥불', nodes:[
      ...
      { key:'f_soot',   name:'그을음 구름', tier:2, max:2, desc:(m)=>'모든 적 이속 -'+R(2*m)+'%, 화상 피해 +'+R(2*m)+'/초 (매캐한 연기가 시야를 태운다)', apply:(p,m)=>{ p.slowAll*=1-0.02*m; p.burnDps=(p.burnDps||6)+2*m; if(!p.burnChance) p.burnChance=0.1; } },
      ...
      // v6.46 하위 테크 5종
      { key:'f_oil',    name:'기름 부음',   tier:1, max:2, desc:(m)=>'화상 확률 +'+R(5*m)+'%p (불붙기 좋은 몸)', apply:(p,m)=>{ p.burnChance=Math.min(0.9,(p.burnChance||0)+0.05*m); if(!p.burnDps) p.burnDps=6; } },
      { key:'f_core',   name:'화염 폭심',   tier:1, max:3, desc:(m)=>'화염구 피해 +'+R(6*m)+', 폭발이 더 뜨겁다', apply:(p,m)=>{ if(!p.fireballLv){ p.fireballLv=1; p.fireballDmg=22; } p.fireballDmg+=6*m; } },
      { key:'f_lava',   name:'용암 발걸음', tier:2, max:2, desc:(m)=>'불의 궤적 지속 +0.3초, 궤적 피해 +'+R(3*m), apply:(p,m)=>{ p.firetrailDur+=0.3; p.firetrailDps=(p.firetrailDps||10)+3*m; if(!p.firetrailLv) p.firetrailLv=1; } },
      { key:'f_sun',    name:'작은 태양',   tier:2, max:2, desc:(m)=>'유성우 피해 +'+R(8*m)+', 화상 피해 +'+R(1*m)+'/초', apply:(p,m)=>{ p.meteorDmg=(p.meteorDmg||20)+8*m; if(!p.meteorLv) p.meteorLv=1; p.burnDps=(p.burnDps||6)+1*m; } },
      { key:'f_rage',   name:'화신의 격노', tier:2, max:2, desc:(m)=>'엘리트·보스 피해 +'+R(6*m)+'%, 화상 피해 +'+R(2*m)+'/초', apply:(p,m)=>{ p.eliteDmg*=1+0.06*m; p.bossDmg*=1+0.06*m; p.burnDps=(p.burnDps||6)+2*m; } },
    ...
    frost: { name:'서리', nodes:[
      ...
      { key:'i_mist',   name:'냉기 안개',   tier:1, max:2, desc:(m)=>'모든 적 이속 -'+R(3*m)+'%, 냉기 중첩 효과 +'+R(1*m)+'%p (안개가 뼛속까지 스민다)', apply:(p,m)=>{ p.slowAll*=1-0.03*m; p.chillPower=Math.min(0.32,(p.chillPower||0.12)+0.01*m); p.chillOn=true; } },
      ...
      { key:'i_veil',   name:'서리 장막',   tier:1, max:2, desc:(m)=>'받는 피해 -'+R(3*m)+'%, 빙결·냉기 적 피해 +'+R(3*m)+'% (얼어붙은 장막 너머로)', apply:(p,m)=>{ p.dmgTaken*=1-0.03*m; p.frozenAmp+=0.03*m; } },
      // v6.46 하위 테크 5종
      { key:'i_needle', name:'서리 바늘',   tier:1, max:3, desc:(m)=>'얼음창 피해 +'+R(5*m)+' (더 가늘고, 더 깊게)', apply:(p,m)=>{ if(!p.lanceLv){ p.lanceLv=1; p.lanceDmg=16; } p.lanceDmg+=5*m; } },
      { key:'i_frostbite',name:'동상',      tier:1, max:2, desc:(m)=>'냉기 중첩 효과 +'+R(2*m)+'%p (손끝부터 얼어붙는다)', apply:(p,m)=>{ p.chillPower=Math.min(0.32,(p.chillPower||0.12)+0.02*m); p.chillOn=true; } },
      { key:'i_glacier',name:'빙하 갑주',   tier:2, max:2, desc:(m)=>'받는 피해 -'+R(2*m)+'%, 서리 갑옷 반응 강화', apply:(p,m)=>{ p.dmgTaken*=1-0.02*m; p.frostArmor=(p.frostArmor||0)+1; } },
      { key:'i_aurora', name:'오로라',      tier:2, max:2, desc:(m)=>'눈보라 피해 +'+R(3*m)+'/초, 쿨다운 -'+R(2*m)+'%', apply:(p,m)=>{ p.blizzDps=(p.blizzDps||6)+3*m; if(!p.blizzLv) p.blizzLv=1; p.cdr*=1-0.02*m; } },
      { key:'i_tomb',   name:'얼음 무덤',   tier:2, max:2, desc:(m)=>'빙결·냉기 적 피해 +'+R(7*m)+'% (무덤은 조용하다)', apply:(p,m)=>{ p.frozenAmp+=0.07*m; } },
    ...
    volt: { name:'번개', nodes:[
      ...
      { key:'l_surge',  name:'서지 전류',   tier:1, max:3, desc:(m)=>'공격속도 +'+R(3*m)+'%, 감전 피해 +'+R(2*m)+' (전류가 몸을 타고 돈다)', apply:(p,m)=>{ p.rateMult*=1+0.03*m; p.shockDmg=(p.shockDmg||10)+2*m; } },
      ...
      // v6.46 하위 테크 5종
      { key:'l_conduct',name:'도체화',      tier:1, max:2, desc:(m)=>'감전 확률 +'+R(5*m)+'%p (몸이 전기를 부른다)', apply:(p,m)=>{ p.shockChance=Math.min(0.8,(p.shockChance||0)+0.05*m); if(!p.shockDmg) p.shockDmg=10; } },
      { key:'l_dynamo', name:'다이나모',    tier:1, max:3, desc:(m)=>'자기장 펄스 피해 +'+R(5*m), apply:(p,m)=>{ p.magfieldDmg=(p.magfieldDmg||9)+5*m; if(!p.magfieldLv) p.magfieldLv=1; } },
      { key:'l_ozone',  name:'오존 냄새',   tier:1, max:2, desc:(m)=>'원소 발동 +'+R(2*m)+'%p, 공격속도 +'+R(2*m)+'% (폭풍 직전의 공기)', apply:(p,m)=>{ p.procBonus=(p.procBonus||0)+0.02*m; p.rateMult*=1+0.02*m; } },
      { key:'l_rail',   name:'레일건 코일', tier:2, max:2, desc:(m)=>'천둥 창 피해 +'+R(7*m)+', 투사체 피해 +'+R(3*m)+'%', apply:(p,m)=>{ p.spearDmg=(p.spearDmg||18)+7*m; if(!p.spearLv) p.spearLv=1; p.projMult*=1+0.03*m; } },
      { key:'l_ion',    name:'이온화',      tier:2, max:2, desc:(m)=>'감전 피해 +'+R(4*m)+', 치명 확률 +'+R(2*m)+'%', apply:(p,m)=>{ p.shockDmg=(p.shockDmg||10)+4*m; p.critChance=Math.min(0.85,p.critChance+0.02*m); } },
    ...
    acid: { name:'부식', nodes:[
      ...
      // v6.46 하위 테크 5종
      { key:'a_catalyst',name:'촉매 주입',  tier:1, max:2, desc:(m)=>'부식 확률 +'+R(4*m)+'%p, 원소 발동 +'+R(2*m)+'%p', apply:(p,m)=>{ p.corrodeChance=Math.min(0.7,p.corrodeChance+0.04*m); p.procBonus=(p.procBonus||0)+0.02*m; } },
      { key:'a_bile',   name:'담즙 순환',   tier:1, max:2, desc:(m)=>'용해 피해 +'+R(2*m)+'/초, 재생 +'+R1(0.2*m)+' (독이 몸을 지킨다)', apply:(p,m)=>{ p.dissolveDps=(p.dissolveDps||0)+2*m; p.regen+=0.2*m; } },
      { key:'a_geyser', name:'산성 간헐천', tier:2, max:2, desc:(m)=>'산성 구름 피해 +'+R(3*m)+'/초, 부식 확률 +'+R(3*m)+'%p', apply:(p,m)=>{ p.acidDps=(p.acidDps||7)+3*m; if(!p.acidLv) p.acidLv=1; p.corrodeChance=Math.min(0.7,p.corrodeChance+0.03*m); } },
      { key:'a_crown',  name:'부식 왕관',   tier:2, max:2, desc:(m)=>'부식 효과 +'+R(2*m)+'%p, 엘리트 피해 +'+R(5*m)+'%', apply:(p,m)=>{ p.corrodeAmp=Math.min(0.4,p.corrodeAmp+0.02*m); p.eliteDmg*=1+0.05*m; } },
      { key:'a_fang',   name:'독니',       tier:2, max:2, desc:(m)=>'산성 파도 피해 +'+R(6*m)+', 용해 피해 +'+R(1*m)+'/초', apply:(p,m)=>{ p.awaveDmg=(p.awaveDmg||14)+6*m; if(!p.awaveLv) p.awaveLv=1; p.dissolveDps=(p.dissolveDps||0)+1*m; } },
    ...
    boom: { name:'폭발', nodes:[
      ...
      { key:'e_plate',  name:'폭연 갑주',   tier:1, max:2, desc:(m)=>'받는 피해 -'+R(3*m)+'%, 돌파 폭발 피해 +'+R(6*m)+' (갑주째로 부딪힌다)', apply:(p,m)=>{ p.dmgTaken*=1-0.03*m; p.dashBlast=(p.dashBlast||20)+6*m; } },
      // v6.46 하위 테크 5종
      { key:'e_trigger',name:'예민한 뇌관', tier:1, max:2, desc:(m)=>'유폭 확률 +'+R(5*m)+'%p, 지뢰 피해 +'+R(4*m), apply:(p,m)=>{ p.explodeChance=Math.min(0.7,(p.explodeChance||0)+0.05*m); p.mineDmg=(p.mineDmg||22)+4*m; if(!p.mineLv) p.mineLv=1; } },
      { key:'e_napalm', name:'네이팜 배합', tier:1, max:3, desc:(m)=>'유폭 폭발 피해 +'+R(8*m)+' (더 넓게, 더 끈질기게)', apply:(p,m)=>{ p.explodeDmg=(p.explodeDmg||18)+8*m; if(!p.explodeChance) p.explodeChance=0.1; } },
      { key:'e_bigone', name:'대형 폭탄',   tier:2, max:2, desc:(m)=>'클러스터 폭격 피해 +'+R(8*m)+', 폭발 피해 +'+R(4*m), apply:(p,m)=>{ p.clusterDmg=(p.clusterDmg||18)+8*m; if(!p.clusterLv) p.clusterLv=1; p.explodeDmg=(p.explodeDmg||18)+4*m; } },
      { key:'e_ring',   name:'폭풍 반경',   tier:2, max:2, desc:(m)=>'돌파 폭발 피해 +'+R(10*m)+', 이동속도 +'+R(2*m)+'%', apply:(p,m)=>{ p.dashBlast=(p.dashBlast||20)+10*m; p.speed*=1+0.02*m; } },
      { key:'e_sapper', name:'공병 수칙',   tier:2, max:2, desc:(m)=>'받는 피해 -'+R(2*m)+'%, 지뢰 피해 +'+R(6*m)+' (매뉴얼대로만 하면 안 죽는다)', apply:(p,m)=>{ p.dmgTaken*=1-0.02*m; p.mineDmg=(p.mineDmg||22)+6*m; if(!p.mineLv) p.mineLv=1; } },
    ...
    mech: { name:'기계', nodes:[
      ...
      { key:'m_battery',name:'보조 배터리', tier:1, max:2, desc:(m)=>'쿨다운 -'+R(3*m)+'%, 터렛 피해 +'+R(2*m)+' (여분의 전력을 터렛에)', apply:(p,m)=>{ p.cdr*=1-0.03*m; p.turretDmg=(p.turretDmg||10)+2*m; } },
      { key:'m_plate',  name:'복합 장갑',   tier:2, max:2, desc:(m)=>'받는 피해 -'+R(3*m)+'%, 체력 +'+R(8*m), apply:(p,m)=>{ p.dmgTaken*=1-0.03*m; p.maxHp+=8*m; } },
      // v6.46 하위 테크 5종
      { key:'m_target', name:'조준 보정',   tier:1, max:2, desc:(m)=>'치명 확률 +'+R(3*m)+'%, 투사체 피해 +'+R(3*m)+'%', apply:(p,m)=>{ p.critChance=Math.min(0.85,p.critChance+0.03*m); p.projMult*=1+0.03*m; } },
      { key:'m_gyro',   name:'자이로 안정기',tier:1, max:2, desc:(m)=>'이동속도 +'+R(3*m)+'%, 공격속도 +'+R(2*m)+'%', apply:(p,m)=>{ p.speed*=1+0.03*m; p.rateMult*=1+0.02*m; } },
      { key:'m_coolant',name:'냉각수 순환', tier:2, max:2, desc:(m)=>'쿨다운 -'+R(2*m)+'%, 터렛 공격속도 +'+R(6*m)+'%', apply:(p,m)=>{ p.cdr*=1-0.02*m; p.turretRate*=1+0.06*m; } },
      { key:'m_drone',  name:'정찰 드론',   tier:2, max:2, desc:(m)=>'수집 범위 +'+R(15*m)+', 골드 +'+R(4*m)+'%', apply:(p,m)=>{ p.magnet+=15*m; p.goldMult*=1+0.04*m; } },
      { key:'m_alloy',  name:'초합금 골격', tier:2, max:2, desc:(m)=>'최대체력 +'+R(10*m)+', 받는 피해 -'+R(2*m)+'%', apply:(p,m)=>{ p.maxHp+=10*m; p.dmgTaken*=1-0.02*m; } },
    ...
    psi: { name:'염동', nodes:[
      ...
      { key:'p_press',  name:'압장 강화',   tier:2, max:2, desc:(m)=>'모든 적 이속 -'+R(3*m)+'%, 염동 파동 피해 +'+R(3*m)+' (짓누르는 사념)', apply:(p,m)=>{ p.slowAll*=1-0.03*m; if(!p.pulseLv){ p.pulseLv=1; p.pulseDmg=14; } p.pulseDmg+=3*m; } },
      ...
      // v6.46 하위 테크 5종
      { key:'p_echo',   name:'사념 메아리', tier:1, max:3, desc:(m)=>'염동 파동 피해 +'+R(4*m)+' (한 번의 생각이 두 번 울린다)', apply:(p,m)=>{ if(!p.pulseLv){ p.pulseLv=1; p.pulseDmg=14; } p.pulseDmg+=4*m; } },
      { key:'p_aegis',  name:'신념의 방패', tier:2, max:2, desc:(m)=>'사이오닉 방벽 충전 -1초', apply:(p,m)=>{ const cd=(p.shieldCdMax||12)-1; p.shieldCdMax=Math.max(4,cd); } },
      { key:'p_insight',name:'통찰',       tier:1, max:2, desc:(m)=>'치명 확률 +'+R(3*m)+'%, 쿨다운 -'+R(2*m)+'%', apply:(p,m)=>{ p.critChance=Math.min(0.85,p.critChance+0.03*m); p.cdr*=1-0.02*m; } },
      { key:'p_distort',name:'공간 왜곡',   tier:2, max:2, desc:(m)=>'회피 +'+R(3*m)+'% (반 발짝 어긋난 곳에 서 있다)', apply:(p,m)=>{ p.dodge=Math.min(0.6,p.dodge+0.03*m); } },
      { key:'p_levit',  name:'부유 명상',   tier:1, max:2, desc:(m)=>'이동속도 +'+R(3*m)+'%, 재생 +'+R1(0.2*m), apply:(p,m)=>{ p.speed*=1+0.03*m; p.regen+=0.2*m; } },
    ...
    holy: { name:'신성', nodes:[
      ...
      { key:'h_armor',  name:'빛의 갑주',   tier:2, max:2, desc:(m)=>'받는 피해 -'+R(3*m)+'%, 회복 효과 +'+R(4*m)+'% (빛이 상처를 어루만진다)', apply:(p,m)=>{ p.dmgTaken*=1-0.03*m; p.healMult*=1+0.04*m; } },
      { key:'h_wrath',  name:'진노',       tier:2, max:2, desc:(m)=>'엘리트·보스 피해 +'+R(8*m)+'%', apply:(p,m)=>{ p.eliteDmg*=1+0.08*m; p.bossDmg*=1+0.08*m; } },
      // v6.46 하위 테크 5종
      { key:'h_candle', name:'봉헌 촛불',   tier:1, max:2, desc:(m)=>'재생 +'+R1(0.3*m)+', 성광 강타 피해 +'+R(3*m), apply:(p,m)=>{ p.regen+=0.3*m; p.smiteDmg=(p.smiteDmg||10)+3*m; if(!p.smiteChance) p.smiteChance=0.08; } },
      { key:'h_hymn',   name:'찬가',       tier:1, max:3, desc:(m)=>'후광 피해 +'+R(5*m)+' (노래가 빛이 된다)', apply:(p,m)=>{ p.haloDmg=(p.haloDmg||12)+5*m; if(!p.haloLv) p.haloLv=1; } },
      { key:'h_censer', name:'향로',       tier:2, max:2, desc:(m)=>'성광 강타 확률 +'+R(4*m)+'%p', apply:(p,m)=>{ p.smiteChance=Math.min(0.7,(p.smiteChance||0)+0.04*m); if(!p.smiteDmg) p.smiteDmg=10; } },
      { key:'h_pilgrim',name:'순례길',     tier:1, max:2, desc:(m)=>'이동속도 +'+R(3*m)+'%, 회복 효과 +'+R(4*m)+'%', apply:(p,m)=>{ p.speed*=1+0.03*m; p.healMult*=1+0.04*m; } },
      { key:'h_martyr', name:'순교자의 각오',tier:2, max:1, desc:(m)=>'피격 무효 확률 +'+R(4*m)+'%p (몸으로 빛을 증명한다)', apply:(p,m)=>{ p.holyWard=Math.min(0.4,(p.holyWard||0)+0.04*m); } },
    ...
    grav: { name:'중력', nodes:[
      ...
      { key:'g_anchor', name:'중력 닻',     tier:1, max:2, desc:(m)=>'받는 피해 -'+R(3*m)+'%, 수집 범위 +'+R(10*m)+' (무거워질수록 끌어당긴다)', apply:(p,m)=>{ p.dmgTaken*=1-0.03*m; p.magnet+=10*m; } },
      { key:'g_lens',   name:'중력 렌즈',   tier:2, max:2, desc:(m)=>'관통 +1, 투사체 +'+R(5*m)+'%', apply:(p,m)=>{ p.pierce+=1; p.projMult*=1+0.05*m; } },
      // v6.46 하위 테크 5종
      { key:'g_tide',   name:'조석력',     tier:1, max:2, desc:(m)=>'중력 우물 피해 +'+R(3*m)+'/초, 적 이속 -'+R(2*m)+'%', apply:(p,m)=>{ p.gravDps=(p.gravDps||8)+3*m; if(!p.gravLv) p.gravLv=1; p.slowAll*=1-0.02*m; } },
      { key:'g_vortex', name:'인력 소용돌이',tier:1, max:2, desc:(m)=>'수집 범위 +'+R(15*m)+', 우물 피해 +'+R(2*m)+'/초', apply:(p,m)=>{ p.magnet+=15*m; p.gravDps=(p.gravDps||8)+2*m; if(!p.gravLv) p.gravLv=1; } },
      { key:'g_press2', name:'중압',       tier:2, max:2, desc:(m)=>'압착 효과 +'+R(6*m)+'% (짓눌린 것은 일어나지 못한다)', apply:(p,m)=>{ p.crushAmp=(p.crushAmp||0)+0.06*m; } },
      { key:'g_dwarf',  name:'백색왜성',   tier:2, max:2, desc:(m)=>'엘리트·보스 피해 +'+R(6*m)+'% (작아진 별이 더 무겁다)', apply:(p,m)=>{ p.eliteDmg*=1+0.06*m; p.bossDmg*=1+0.06*m; } },
      { key:'g_disk',   name:'강착원반',   tier:1, max:2, desc:(m)=>'골드 +'+R(5*m)+'%, 수집 범위 +'+R(10*m)+' (떨어지는 것은 전부 내 것)', apply:(p,m)=>{ p.goldMult*=1+0.05*m; p.magnet+=10*m; } },
    ...
    chrono: { name:'시간', nodes:[
      ...
      { key:'c_wind',   name:'태엽 감기',   tier:1, max:3, desc:(m)=>'공격속도 +'+R(4*m)+'%, 정지 상태의 적 피해 +'+R(3*m)+'% (태엽이 감길수록 빨라진다)', apply:(p,m)=>{ p.rateMult*=1+0.04*m; p.frozenAmp+=0.03*m; } },
      ...
      // v6.46 하위 테크 5종
      { key:'c_gear',   name:'톱니 맞물림', tier:1, max:2, desc:(m)=>'공격속도 +'+R(3*m)+'%, 정지 확률 +'+R(2*m)+'%p', apply:(p,m)=>{ p.rateMult*=1+0.03*m; p.stutterChance=Math.min(0.6,(p.stutterChance||0)+0.02*m); p.stutterDur=p.stutterDur||0.5; } },
      { key:'c_sand',   name:'모래시계 파편',tier:1, max:2, desc:(m)=>'쿨다운 -'+R(2*m)+'%, 회피 +'+R(2*m)+'%', apply:(p,m)=>{ p.cdr*=1-0.02*m; p.dodge=Math.min(0.6,p.dodge+0.02*m); } },
      { key:'c_rewind', name:'되감기 태엽', tier:2, max:2, desc:(m)=>'재생 +'+R1(0.3*m)+' (상처가 아물던 순간으로)', apply:(p,m)=>{ p.regen+=0.3*m; } },
      { key:'c_future', name:'미래시',     tier:2, max:2, desc:(m)=>'치명 확률 +'+R(4*m)+'% (0.5초 뒤를 본다)', apply:(p,m)=>{ p.critChance=Math.min(0.85,p.critChance+0.04*m); } },
      { key:'c_bubble', name:'시간 거품',   tier:2, max:2, desc:(m)=>'지연장 감속 +'+R(8*m)+'%p, 받는 피해 -'+R(2*m)+'%', apply:(p,m)=>{ p.dragField=Math.min(0.7,(p.dragField||0)+0.08*m); p.dmgTaken*=1-0.02*m; } },
    ...
    blood: { name:'혈마', nodes:[
      ...
      { key:'b_iron',   name:'철분 과다',   tier:1, max:2, desc:(m)=>'받는 피해 -'+R(3*m)+'%, 최대체력 +'+R(6*m)+' (피가 쇠처럼 굳는다)', apply:(p,m)=>{ p.dmgTaken*=1-0.03*m; p.maxHp+=6*m; } },
      ...
      // v6.46 하위 테크 5종
      { key:'b_scent',  name:'피 냄새',    tier:1, max:2, desc:(m)=>'수집 범위 +'+R(12*m)+', 이동속도 +'+R(2*m)+'% (피 냄새를 쫓는다)', apply:(p,m)=>{ p.magnet+=12*m; p.speed*=1+0.02*m; } },
      { key:'b_ritual', name:'소혈 의식',   tier:1, max:2, desc:(m)=>'혈폭 확률 +'+R(5*m)+'%p', apply:(p,m)=>{ p.bloodBurstCh=Math.min(0.7,(p.bloodBurstCh||0)+0.05*m); if(!p.bloodBurstDmg) p.bloodBurstDmg=14; } },
  ...
  const NODE_CAT = {
    ...
    // v6.46 신규 55노드
    f_oil:'전술', f_core:'무기', f_lava:'무기', f_sun:'무기', f_rage:'전술',
    i_needle:'무기', i_frostbite:'전술', i_glacier:'수호', i_aurora:'무기', i_tomb:'전술',
    l_conduct:'전술', l_dynamo:'무기', l_ozone:'보조', l_rail:'무기', l_ion:'전술',
    a_catalyst:'전술', a_bile:'보조', a_geyser:'무기', a_crown:'전술', a_fang:'무기',
    e_trigger:'전술', e_napalm:'무기', e_bigone:'무기', e_ring:'무기', e_sapper:'수호',
    m_target:'전술', m_gyro:'보조', m_coolant:'보조', m_drone:'운명', m_alloy:'수호',
    p_echo:'무기', p_aegis:'수호', p_insight:'전술', p_distort:'수호', p_levit:'보조',
    h_candle:'보조', h_hymn:'무기', h_censer:'전술', h_pilgrim:'보조', h_martyr:'수호',
  ...
  Object.keys(CGW_NAMES).forEach((ck)=>{
    ...
    (PGW_SUFFIX_OVR[ck]||PGW_SUFFIX[g]).forEach((sfx, i)=>{
      ...
      WEAPONS[key] = {
        ...
        dmg:(w)=>{
          // 만렙 50 곡선: 초반 일반 무기 이하 → Lv15/30/45 왕귀. 원형 밸런스 계수로 3종 성능 동일
          const gl = (DB.pgw && DB.pgw[ck+'_'+i] && DB.pgw[ck+'_'+i].lv)||1;
          return (1.6 + gl*0.5) * (ARCH_BAL[arch]||1) * [1,1.25,1.55,1.9,2.3][w.lv-1] * (gl>=45?2.4: gl>=30?1.7: gl>=15?1.25:1) * (w.evolved?1.5:1);
        },
  ...
  function rollUpgrades(n){
    ...
    if (player.weapons.length < (player.weaponCap||MAX_WEAPONS)){
      ...
      Object.keys(WEAPONS).forEach((key)=>{
        ...
        if (['nameless','gbow','gtome','gblade'].includes(key) || key.indexOf('cgw_')===0 || key.indexOf('pgw_')===0) return;
  ...
  function openMerchant(){
    ...
    // (영구강화 상점 공방 폐지 — 획득은 오직 이 만남뿐)
    {
      ...
      if (LGW && !DB.gweps[LGW.k].found && (DB.peril||0)>=12 && Math.random()<0.02){
        const costL = Math.round(4000 * (player.merchantDisc||1));
        opts.push({
          l:'✦ '+LGW.n+' ('+costL+'G)', d:LGW.d,
          fx:()=>{
            if (runGold < costL){ toast('골드 부족!'); SFX.play('hit'); return; }
            runGold -= costL;
            DB.gweps[LGW.k].found = true;
            unlockAch('craft1');
            saveDB();
            toast('✦ ['+LGW.n+'] 을(를) 손에 넣었다! 레벨업에서 선택 가능');
            SFX.play('evolve');
            updateHud();
          }
        });
  ...
  const FIELD_EVENTS = [
    ...
    // v6.46 신규 이벤트 5종
    { t:'길 잃은 보물 골렘', d:'무리에서 떨어진 새끼 보물 골렘이 두리번거린다. 등에 톱니가 박혀 있다.', opts:[
      { l:'등의 톱니를 뽑는다', d:'50% ⚙ 고대 톱니 +1 / 50% 골렘이 울며 동료를 부른다', fx:()=>{
          if (Math.random()<0.5){ DB.mats.gear=(DB.mats.gear||0)+1; saveDB(); toast('⚙ 고대 톱니 +1'); SFX.play('chest'); }
          else { toast('끼이잉—! 동료 골렘의 습격!'); for (let k=0;k<7;k++){ const a=Math.random()*Math.PI*2; enemies.push(makeEnemy('normal', player.x+Math.cos(a)*340, player.y+Math.sin(a)*340, false)); } SFX.play('warn'); } } },
      { l:'집까지 바래다준다', d:'사례로 골드 +40~80', fx:()=>{ const g=gainGold(40+((Math.random()*41)|0)); toast('골렘의 사례 +'+g+'G'); SFX.play('coin'); } },
    ]},
    { t:'피에 굶주린 제단', d:'검붉은 제단이 낮게 고동친다. 바치라는 뜻이다.', opts:[
      { l:'피를 바친다', d:'체력 -35% → 치명타 확률 +8%p', fx:()=>{ player.hp=Math.max(1,player.hp-player.maxHp*0.35); player.critChance=Math.min(0.9,(player.critChance||0)+0.08); toast('제단이 만족했다 (치명타 +8%p)'); SFX.play('quest'); } },
      { l:'제단을 닦아낸다', d:'덕을 쌓는다 (재생 +0.4)', fx:()=>{ player.regen+=0.4; toast('마음이 가볍다 (+재생)'); SFX.play('pick'); } },
    ]},
    { t:'떠돌이 연금술사', d:'솥을 짊어진 연금술사. "방금 완성된 물약인데... 뭐가 나올진 나도 몰라."', opts:[
      { l:'물약을 산다 (50G)', d:'40% 최대체력 +8% / 40% 체력 60% 회복 / 20% 실패작 (체력 -15%)', fx:()=>{
          if (runGold>=50){ runGold-=50; } else { DB.gold=Math.max(0,DB.gold-50); }
          const r3=Math.random();
          if (r3<0.4){ player.maxHp=Math.round(player.maxHp*1.08); player.hp=Math.min(player.maxHp,player.hp+15); toast('몸이 단단해졌다 (+최대체력 8%)'); SFX.play('quest'); }
          else if (r3<0.8){ player.hp=Math.min(player.maxHp, player.hp+player.maxHp*0.6); toast('쭉— 들이켰다 (체력 60%)'); SFX.play('pick'); }
          else { player.hp=Math.max(1,player.hp-player.maxHp*0.15); toast('웩! 실패작이다'); SFX.play('hit'); } } },
      { l:'조수를 돕는다', d:'재료 줍는 법을 배운다 (수집 +30)', fx:()=>{ player.magnet+=30; toast('수집 반경 +30'); SFX.play('pick'); } },
    ]},
    { t:'낡은 훈련 허수아비', d:'전장 한복판에 세워진 허수아비. 아직 쓸 만해 보인다.', opts:[
      { l:'두들긴다', d:'감이 잡힌다 — 경험치 획득', fx:()=>{ grantXp(Math.ceil(player.xpNext*0.5)); toast('수련의 성과!'); SFX.play('pick'); } },
      { l:'해체한다', d:'30% 강화석 / 70% 골드 +20', fx:()=>{
          if (Math.random()<0.3){ dropItem(player.x+40, player.y, 'whet'); toast('속에서 강화석이!'); SFX.play('chest'); }
          else { const g=gainGold(20); toast('고철값 +'+g+'G'); SFX.play('coin'); } } },
    ]},
    { t:'잠든 미믹 상자', d:'상자가... 숨을 쉬고 있다. 자는 틈에 열어볼까.', opts:[
      { l:'조심스레 연다', d:'45% 희귀+ 장비 / 55% 미믹 각성 (습격)', fx:()=>{
          if (Math.random()<0.45){ addEquip(genEquip(2)); toast('자는 미믹의 뱃속에서 장비를!'); SFX.play('chest'); }
          else { toast('미믹이 깨어났다!!'); for (let k=0;k<6;k++){ const a=Math.random()*Math.PI*2; enemies.push(makeEnemy('normal', player.x+Math.cos(a)*160, player.y+Math.sin(a)*160, false)); } SFX.play('warn'); } } },
      { l:'살금살금 물러난다', d:'긴장이 풀린다 (5초 이속 +20%)', fx:()=>{ tbuff('spd',1.2,5); toast('휴... 발걸음이 가볍다'); SFX.play('pick'); } },
    ]},
  ];
  ...
  function addTextNum(x, y, text){
    if (dmgNums.length > 80) dmgNums.shift();
    // v6.68 겹침 분산: 근처에 살아있는 라벨이 있으면 위로 밀어 세로 쌓임을 줄인다 (모바일 가독)
    let stack = 0;
    for (const n of dmgNums){
      if (n.label && n.age < 0.45 && Math.abs(n.x-x) < 90 && Math.abs(n.y-(y-6)) < 34) stack++;
    }
    dmgNums.push({ x, y:y-6-stack*15, vy:-40, life:0.9, age:0, text, crit:false, label:true });
  }
  ...
  function makeEnemy(type, x, y, elite){
    ...
    let e;
    if (type==='swarm'){
      e = { type, x,y, r:8, hp:7*s, maxHp:7*s, speed:84+Math.random()*20, dmg:5*ds, xpValue:1, hitCd:0 }; // 파리떼 속도 너프
    } else if (type==='brute'){
  ...
  // v6.67 관문 클린 스테이지: 관문 기믹 외 필드 요소 전면 정리 — 결계 안에선 보스에만 집중
  function clearFieldForGate(){
    surveys.length = 0; altars.length = 0; rifts.length = 0;
    clients.length = 0; merchants.length = 0;
    hazards.length = 0; hostileShots.length = 0;
    toast('⚔ 결계가 펼쳐진다 — 관문 전투에 집중하라');
  }
  // v6.67 체력비례 기믹 이벤트 (관문 본보스 공통): 75%/50%/25% 도달 시 각 1회 — 홀드/연타/선택 인터랙션
  function gateHpEvent(b, type, ds2){
    freeze = Math.max(freeze, 0.25);
    const a2 = Math.random()*Math.PI*2;
    const ox = player.x+Math.cos(a2)*180, oy = player.y+Math.sin(a2)*180;
    if (type==='hold'){
      showBossBanner('약점 노출', '"...갑주 틈에서 빛이 샌다."', '#3f7a5c');
      addGateObj({ kind:'hold', icon:'⚡', x:ox, y:oy, r:34, maxT:5.5, holdNeed:1.2,
        onTap:()=>{
          b.hp -= b.maxHp*0.06;
          addDmgNum(b.x, b.y, b.maxHp*0.06, true);
          addTextNum(b.x, b.y-b.r-16, '약점 파훼!');
          effects.push({ type:'ring', x:b.x, y:b.y, life:0.4, age:0, r0:b.r*0.5, r1:b.r*2.2, col:'#3f7a5c' });
          refreshBossBar();
          if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); }
        },
        onFail:()=>{ for (let k=0;k<12;k++){ const a3=(Math.PI*2/12)*k; hostileShot(b.x, b.y, a3, 190, 6, 20*ds2, 2.4); } addTextNum(player.x, player.y-40, '파훼 실패 — 반격!'); gimFail(b, ds2); } });
    } else if (type==='mash'){
      showBossBanner('집중', '"...정신이 한 점으로 모인다."', '#3f7a5c');
      addGateObj({ kind:'mash', icon:'❗', x:ox, y:oy, r:30, maxT:4.5, need:5,
        onTap:()=>{
          b.hp -= b.maxHp*0.04;
          player.skCds = [0,0,0,0];
          addDmgNum(b.x, b.y, b.maxHp*0.04, true);
          addTextNum(player.x, player.y-30, '캐스팅 붕괴 — 스킬 재장전!');
          refreshBossBar();
          if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); }
        },
        onFail:()=>{ for (let k=0;k<16;k++){ const a3=(Math.PI*2/16)*k; hostileShot(b.x, b.y, a3, 170, 6, 22*ds2, 2.6); } addTextNum(player.x, player.y-40, '집중 실패 — 광역 폭발!'); gimFail(b, ds2); } });
    } else {
      showBossBanner('최후의 계약', '"선택하라."', '#8a6a3f');
      b.gexPactDone = false;
      const mkPact = (icon, dx2, fx2)=>addGateObj({ kind:'pick', icon, x:player.x+dx2, y:player.y-70, r:24, maxT:5,
        onPick:()=>{ if (b.gexPactDone) return; b.gexPactDone = true; fx2(); },
        onFail:()=>{
          if (!b.gexPactDone && !b.gexHealed){
            b.gexHealed = true;
            b.hp = Math.min(b.maxHp, b.hp + b.maxHp*0.1);
            refreshBossBar();
            addTextNum(b.x, b.y-b.r-16, '계약 결렬 — 보스가 회복한다');
            SFX.play('warn');
            gimFail(b, ds2);
          }
        } });
      mkPact('🗡', -95, ()=>{ b.hp -= b.maxHp*0.04; player.dmgMult *= 1.12; addTextNum(player.x, player.y-30, '🗡 공세 계약 — 피해 +12%'); refreshBossBar(); });
      mkPact('🛡', 95, ()=>{ player.dmgTaken *= 0.85; addTextNum(player.x, player.y-30, '🛡 수호 계약 — 받는 피해 -15%'); });
    }
    SFX.play('warn');
  }
  // v6.67 스페셜 기믹 3종 (타 게임 오마주 — 관문 본보스당 1회, 100초 시점 랜덤)
  function spGimDarkness(b, ds2){ // FF14 엑사플레어풍: 어둠 속, 금빛 링이 감싼 안전지대 한 곳
    showBossBanner('암전', '"빛이 꺼진다..."', '#5c4a8a');
    screenDimT = Math.max(screenDimT, 3);
    const sa = Math.random()*Math.PI*2;
    const sx = player.x+Math.cos(sa)*140, sy = player.y+Math.sin(sa)*140;
    for (let gx=-2;gx<=2;gx++){
      for (let gy=-2;gy<=2;gy++){
        const hx = player.x+gx*110, hy = player.y+gy*110;
        if (Math.hypot(hx-sx, hy-sy) < 90) continue;
        addHazard(hx, hy, 58, 2.2, 26*ds2, false);
      }
    }
    effects.push({ type:'ring', x:sx, y:sy, life:2.2, age:0, r0:82, r1:86, col:'#e8c56a' });
    SFX.play('warn');
  }
  function spGimSeq(b, ds2){ // 로아 큐브·FF14 타워풍: 봉인 인장을 순서대로
    showBossBanner('봉인의 인장', '"순서를 기억하는가."', '#8a6a3f');
    const grp = 'sg'+Math.floor(performance.now());
    let wrongFired = false;
    for (let k=0;k<3;k++){
      const a2 = Math.random()*Math.PI*2;
      addGateObj({ kind:'seq', grp, seq:k+1, icon:['①','②','③'][k],
        x:player.x+Math.cos(a2)*(120+k*60), y:player.y+Math.sin(a2)*(120+k*60), r:24, maxT:8,
        onTap:()=>{
          b.hp -= b.maxHp*0.07;
          addDmgNum(b.x, b.y, b.maxHp*0.07, true);
          addTextNum(b.x, b.y-b.r-16, '봉인 파훼!');
          refreshBossBar();
          if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); }
        },
        onWrong:()=>{
          if (wrongFired) return; wrongFired = true;
          for (let k2=0;k2<14;k2++){ const a3=(Math.PI*2/14)*k2; hostileShot(b.x, b.y, a3, 180, 6, 22*ds2, 2.4); }
          addTextNum(player.x, player.y-40, '봉인이 뒤틀렸다!');
          gimFail(b, ds2);
        },
        onFail:()=>{} });
    }
  ...
  function spGimBomb(b, ds2){ // WoW 살아있는 폭탄풍: 붙은 폭탄을 보스 곁에서 터뜨려 되돌려줘라
    showBossBanner('시한폭탄', '"이건... 나한테 붙었잖아?!"', '#b8362e');
    player.gimBombT = 5;
    player.gimBombBoss = b;
    SFX.play('warn');
  }
  function spGimMark(b, ds2){ // WoW 죽음의 낙인풍: 낙인이 찍히면 발밑이 계속 썩는다 — 멈추지 마라
    showBossBanner('낙인', '"...발밑이 썩어간다."', '#7a4fa8');
    player.gimMarkT = 5;
    SFX.play('warn');
  }
  function spGimGravity(b, ds2){ // WoW 중력탄풍: 보스가 끌어당긴다 — 필사적으로 벗어나라
    showBossBanner('인력', '"...이리 와."', '#6a5acd');
    player.gimGravT = 3;
    player.gimGravBoss = b;
    SFX.play('warn');
  }
  function spGimStill(b, ds2){ // 로아 일리아칸풍 '멈춰라': 시선이 꽂히면 움직임이 곧 상처
    showBossBanner('정지 명령', '"움직이지 마라."', '#b8362e');
    player.gimStillT = 2.5;
    SFX.play('warn');
  }
  function spGimAdds(b, ds2){ // WoW 애드 페이즈풍: 그림자를 시간 내 정화하라 — 남기면 보스가 흡수한다
    showBossBanner('그림자 소환', '"나의 그림자들이여."', '#55486a');
    for (let k=0;k<3;k++){
      const a2 = (Math.PI*2/3)*k;
      const ad = makeEnemy('swarm', b.x+Math.cos(a2)*150, b.y+Math.sin(a2)*150, false);
      ad.spAdd = true; ad.name = '정화되지 않은 그림자';
      ad.hp *= 8; ad.maxHp *= 8; ad.grade = 0; ad.xpValue = 0;
      enemies.push(ad);
    }
    b.spAddT = 9;
    SFX.play('warn');
  }
  function spGimCross(b, ds2){ // FF14 십자 장판풍: 십자를 벗어나 대각으로
    showBossBanner('십자 포화', '"피할 곳은 모서리뿐."', '#c9403a');
    addHazard(player.x, player.y, 52, 1.1, 22*ds2, false);
    for (let d2=0;d2<4;d2++){
      const a2 = d2*Math.PI/2;
      for (let k=1;k<=5;k++) addHazard(player.x+Math.cos(a2)*k*80, player.y+Math.sin(a2)*k*80, 52, 1.1+k*0.08, 22*ds2, false);
    }
    SFX.play('warn');
  }
  function spGimSweep(b, ds2){ // FF14 회전초침풍: 부채꼴이 시계방향으로 차례로 터진다 — 함께 돌아라
    showBossBanner('회전 초침', '"시간은 돌고 돈다."', '#5ab8c9');
    for (let k=0;k<8;k++){
      const a2 = (Math.PI*2/8)*k;
      for (let r2=1;r2<=2;r2++) addHazard(b.x+Math.cos(a2)*r2*95, b.y+Math.sin(a2)*r2*95, 56, 0.8+k*0.35, 24*ds2, false);
    }
    SFX.play('warn');
  }
  function spGimDist(b, ds2){ // 로아 카양겔풍 거리 심판: 판정 순간 곁에 있으면 벌을 받는다
    showBossBanner('심판 — 거리', '"내 곁은 지옥이다."', '#8a6a3f');
    b.spDistT = 2.5;
    SFX.play('warn');
  }
  function spGimAltar(b, ds2){ // 디아블로풍 피의 제단: 내 피를 바쳐 적의 격을 꺾는다 (선택)
    showBossBanner('피의 제단', '"바칠 것인가."', '#c9403a');
    const a2 = Math.random()*Math.PI*2;
    addGateObj({ kind:'pick', icon:'🩸', x:player.x+Math.cos(a2)*200, y:player.y+Math.sin(a2)*200, r:24, maxT:5,
      onPick:()=>{
        const cost = Math.round(player.maxHp*0.2);
        player.hp = Math.max(1, player.hp - cost);
        b.hp -= b.maxHp*0.12;
        addDmgNum(b.x, b.y, b.maxHp*0.12, true);
        addTextNum(player.x, player.y-30, '-'+cost+' 피의 대가');
        refreshBossBar();
        if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); }
      },
      onFail:()=>{} });
  ...
  function spGimThunder(b, ds2){ // WoW 유도 낙뢰풍: 내 자리에 떨어진다 — 보스 곁으로 유도해 같이 맞혀라
    showBossBanner('천벌', '"하늘이 겨눈다."', '#e0b73d');
    b.spThunN = 3; b.spThunT = 1.2;
    SFX.play('warn');
  }
  function spGimGaze(b, ds2){ // 시선 사격풍: 조준선이 그어진다 — 조준이 끝나기 전에 측면으로
    showBossBanner('응시', '"보고 있다."', '#9a6fc4');
    b.spGazeT = 1.2;
    b.spGazeA = Math.atan2(player.y-b.y, player.x-b.x);
    SFX.play('warn');
  }
  function spGimRSeq(b, ds2){ // 역순의 인장: 끝 번호부터 — 순서 감각을 뒤집는다
    showBossBanner('역순의 인장', '"끝에서부터."', '#8a6a3f');
    const grp = 'rs'+Math.floor(performance.now());
    let wrongFired = false;
    for (let k=0;k<3;k++){
      const a2 = Math.random()*Math.PI*2;
      addGateObj({ kind:'seq', grp, seq:3-k, icon:['①','②','③'][k],
        x:player.x+Math.cos(a2)*(120+k*60), y:player.y+Math.sin(a2)*(120+k*60), r:24, maxT:8,
        onTap:()=>{ b.hp -= b.maxHp*0.07; addDmgNum(b.x, b.y, b.maxHp*0.07, true); addTextNum(b.x, b.y-b.r-16, '역봉인 파훼!'); refreshBossBar(); if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); } },
        onWrong:()=>{ if (wrongFired) return; wrongFired = true; for (let k2=0;k2<14;k2++){ const a3=(Math.PI*2/14)*k2; hostileShot(b.x, b.y, a3, 180, 6, 22*ds2, 2.4); } addTextNum(player.x, player.y-40, '봉인이 뒤틀렸다!'); gimFail(b, ds2); },
        onFail:()=>{} });
    }
    SFX.play('warn');
  }
  function spGimDefuse(b, ds2){ // 폭탄 해체풍: 두 곳의 폭탄을 시간 내 전부 — 동선 설계
    showBossBanner('이중 폭탄', '"어느 쪽부터?"', '#b8362e');
    let left = 2, failed = false;
    for (let k=0;k<2;k++){
      const a2 = Math.random()*Math.PI + k*Math.PI;
      addGateObj({ kind:'qte', icon:'💣', x:player.x+Math.cos(a2)*190, y:player.y+Math.sin(a2)*190, r:22, maxT:5,
        onTap:()=>{ left--; if (left===0){ b.hp -= b.maxHp*0.06; addDmgNum(b.x, b.y, b.maxHp*0.06, true); addTextNum(player.x, player.y-30, '해체 완료!'); refreshBossBar(); if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); } } },
        onFail:()=>{ if (failed) return; failed = true; for (let k2=0;k2<18;k2++){ const a3=(Math.PI*2/18)*k2; hostileShot(player.x, player.y, a3, 190, 6, 24*ds2, 2.2); } addTextNum(player.x, player.y-40, '폭탄이 터졌다!'); gimFail(b, ds2); } });
    }
    SFX.play('warn');
  }
  function spGimRhythm(b, ds2){ // 리듬 탭풍: 링이 닿는 정박에 맞춰
    showBossBanner('박자', '"...하나, 둘."', '#c9895a');
    const a2 = Math.random()*Math.PI*2;
    addGateObj({ kind:'rhythm', icon:'♪', x:player.x+Math.cos(a2)*160, y:player.y+Math.sin(a2)*160, r:22, maxT:2.4, window:0.45,
      onTap:()=>{ b.hp -= b.maxHp*0.07; addDmgNum(b.x, b.y, b.maxHp*0.07, true); addTextNum(player.x, player.y-30, '퍼펙트!'); refreshBossBar(); if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); } },
      onFail:()=>{ for (let k2=0;k2<10;k2++){ const a3=(Math.PI*2/10)*k2; hostileShot(b.x, b.y, a3, 175, 6, 20*ds2, 2.2); } gimFail(b, ds2); } });
    SFX.play('warn');
  }
  function spGimFind(b, ds2){ // 로아 아브렐풍 진짜 찾기: 미세한 금빛 맥동이 단서
    showBossBanner('허상', '"어느 쪽이 진짜인가."', '#55486a');
    const grp = 'fd'+Math.floor(performance.now());
    const realIdx = (Math.random()*3)|0;
    for (let k=0;k<3;k++){
      const a2 = (Math.PI*2/3)*k + Math.random()*0.4;
      addGateObj({ kind:'find', grp, real:k===realIdx, icon:'👁',
        x:player.x+Math.cos(a2)*180, y:player.y+Math.sin(a2)*180, r:22, maxT:6,
        onTap:()=>{ b.hp -= b.maxHp*0.07; addDmgNum(b.x, b.y, b.maxHp*0.07, true); addTextNum(player.x, player.y-30, '간파!'); refreshBossBar(); if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); } },
        onWrong:()=>{ for (let k2=0;k2<12;k2++){ const a3=(Math.PI*2/12)*k2; hostileShot(b.x, b.y, a3, 180, 6, 22*ds2, 2.4); } addTextNum(player.x, player.y-40, '허상이었다!'); gimFail(b, ds2); },
        onFail:()=>{} });
    }
    SFX.play('warn');
  }
  function spGimShove(b, ds2){ // 드래그 밀치기풍: 힘 대 힘 — 보스 쪽으로 화면을 힘껏 그어라
    showBossBanner('힘겨루기', '"밀리지 마라."', '#e2823f');
    player.gimShoveT = 4;
    player.gimShoveBoss = b;
    SFX.play('warn');
  }
  // v6.68 스페셜 기믹 풀(18종) — 전투당 2회(100초/140초), 같은 것 반복 없음. 풀은 계속 확장한다
  function runSpecialGim(b, ds2){
    const pool = ['dark','seq','bomb','mark','grav','still','adds','cross','sweep','dist','altar','thunder','gaze','rseq','defuse','rhythm','find','shove']
      .filter(k=>!(b.spUsed||[]).includes(k));
    const pick = pool[(Math.random()*pool.length)|0];
    (b.spUsed = b.spUsed||[]).push(pick);
    if (pick==='dark') spGimDarkness(b, ds2);
    else if (pick==='seq') spGimSeq(b, ds2);
    else if (pick==='bomb') spGimBomb(b, ds2);
    else if (pick==='mark') spGimMark(b, ds2);
    else if (pick==='grav') spGimGravity(b, ds2);
    else if (pick==='still') spGimStill(b, ds2);
    else if (pick==='adds') spGimAdds(b, ds2);
    else if (pick==='cross') spGimCross(b, ds2);
    else if (pick==='sweep') spGimSweep(b, ds2);
    else if (pick==='dist') spGimDist(b, ds2);
    else if (pick==='altar') spGimAltar(b, ds2);
    else if (pick==='thunder') spGimThunder(b, ds2);
    else if (pick==='gaze') spGimGaze(b, ds2);
    else if (pick==='rseq') spGimRSeq(b, ds2);
    else if (pick==='defuse') spGimDefuse(b, ds2);
    else if (pick==='rhythm') spGimRhythm(b, ds2);
    else if (pick==='find') spGimFind(b, ds2);
    else spGimShove(b, ds2);
  }
  // v6.68 기믹 실패 스택제: 단발 실패는 용서하되, 3회 쌓이면 '한계'가 온다
  function gimFail(b, ds2){
    b.gimFailN = (b.gimFailN||0)+1;
    if (b.gimFailN>=3 && !b.gimLimitFired){
      b.gimLimitFired = true;
      showBossBanner('한계', '"세 번이나 놓쳤다."', '#b8362e');
      for (let k=0;k<36;k++){ const a3=(Math.PI*2/36)*k; hostileShot(b.x, b.y, a3, 165, 7, 30*ds2, 3.4); }
  ...
  function tickGateCore(b, dt){
    ...
    if (b.gcCoreT<=0){
      ...
      if (roll < 0.5){
        addGateObj({ kind:'qte', icon:'💥', x:ox, y:oy, r:23, maxT:3.5,
          ...
          onFail:()=>{} });
        addTextNum(player.x, player.y-44, '💥 무언가 드러났다...');
      } else if (roll < 0.8){
        addGateObj({ kind:'qte', icon:'⚡', x:ox, y:oy, r:23, maxT:3,
          ...
          onFail:()=>{} });
        addTextNum(player.x, player.y-44, '⚡ 불안정한 마력이 감돈다');
      } else {
        addGateObj({ kind:'qte', icon:'🛡', x:ox, y:oy, r:23, maxT:3,
          onTap:()=>{ player.invuln=Math.max(player.invuln,2); addTextNum(player.x, player.y-30, '🛡 잔향 보호막 (2초)'); },
          onFail:()=>{} });
        addTextNum(player.x, player.y-44, '🛡 잔향이 맴돈다');
      }
      SFX.play('tele');
    }
    // v6.67 체력비례 기믹 이벤트 (본보스 전용): 75%/50%/25% 도달 시 각 1회 — 조건 기반 트리거
    if (b.finale){
      const frac2 = b.hp/(b.maxHp||1);
      b.gexDone = b.gexDone||{};
      if (frac2<=0.75 && !b.gexDone.a){ b.gexDone.a = true; gateHpEvent(b, 'hold', ds2); }
      if (frac2<=0.5  && !b.gexDone.b){ b.gexDone.b = true; gateHpEvent(b, 'mash', ds2); }
      if (frac2<=0.25 && !b.gexDone.c){ b.gexDone.c = true; gateHpEvent(b, 'choice', ds2); }
      // v6.68 공통 압박 탄막: 페이즈가 깊어질수록 조준 부채살 + 링 탄막이 거세진다 (전 본보스 난이도 상향)
      b.gcPressT = (b.gcPressT===undefined?5:b.gcPressT) - dt;
      if (b.gcPressT<=0){
        const frac3 = b.hp/(b.maxHp||1);
        const tier = frac3<0.33?3 : frac3<0.66?2 : 1;
        const aim3 = Math.atan2(player.y-b.y, player.x-b.x);
        for (let k=-tier;k<=tier;k++) hostileShot(b.x, b.y, aim3+k*0.14, 250+tier*15, 6, 16*ds2, 2.4);
        if (tier>=2){
          const nR = 8+tier*4;
          for (let k=0;k<nR;k++){ const a4=(Math.PI*2/nR)*k + Math.random()*0.2; hostileShot(b.x, b.y, a4, 150+tier*12, 6, 14*ds2, 3); }
        }
        b.gcPressT = 6.5 - tier*1.2;
      }
      // v6.68 그림자 정화 판정: 시간 내 전멸 = 보스 5% 삭제 / 실패 = 보스가 그림자를 흡수해 8% 회복
      if (b.spAddT>0){
        b.spAddT -= dt;
        const alive2 = enemies.some(e2=>e2.spAdd);
        if (!alive2){
          b.spAddT = 0;
          b.hp -= b.maxHp*0.05;
          addDmgNum(b.x, b.y, b.maxHp*0.05, true);
          addTextNum(b.x, b.y-b.r-16, '정화 완료!');
          refreshBossBar();
        } else if (b.spAddT<=0){
          b.hp = Math.min(b.maxHp, b.hp + b.maxHp*0.08);
          addTextNum(b.x, b.y-b.r-16, '그림자를 흡수했다...');
          refreshBossBar();
          gimFail(b, ds2);
        }
      }
      ...
      if (b.spDistT>0){
        b.spDistT -= dt;
        if (b.spDistT<=0){
          if (Math.hypot(player.x-b.x, player.y-b.y) < 240){
            addTextNum(player.x, player.y-40, '심판 — 너무 가까웠다!');
            playerHit(35*ds2, 0.5, 12);
          } else addTextNum(player.x, player.y-40, '심판 회피!');
        }
      }
      // v6.68 천벌 3연: 내 자리에 낙뢰 — 보스 곁이면 보스도 같이 맞는다
      if (b.spThunN>0){
        b.spThunT -= dt;
        if (b.spThunT<=0){
          b.spThunN--; b.spThunT = 1.4;
          addHazard(player.x, player.y, 70, 0.8, 26*ds2, false);
          effects.push({ type:'bolt', x:player.x, y:player.y, life:0.22, age:0 });
          if (Math.hypot(player.x-b.x, player.y-b.y) < 160){
            b.hp -= b.maxHp*0.04;
            addDmgNum(b.x, b.y, b.maxHp*0.04, true);
            addTextNum(b.x, b.y-b.r-16, '같이 맞았다!');
            refreshBossBar();
            if (b.hp<=0){ const bi=bosses.indexOf(b); if (bi>=0) defeatBoss(bi); return; }
          }
          SFX.play('warn');
        }
      }
      // v6.68 응시 발사: 조준선이 사라지는 순간 관통 5연
      if (b.spGazeT>0){
        b.spGazeT -= dt;
        if (b.spGazeT<=0){
          for (let k=0;k<5;k++) hostileShot(b.x, b.y, b.spGazeA + (Math.random()-0.5)*0.05, 420+k*30, 7, 26*ds2, 1.6);
          SFX.play('sweep');
        }
      }
      // v6.68 스페셜 기믹 (타 게임 오마주 18종 로테이션): 100초 / 140초 — 전투당 2회, 반복 없음
      if ((b.aliveT||0) > 100 && !b.spGimDone){ b.spGimDone = true; runSpecialGim(b, ds2); }
      if ((b.aliveT||0) > 140 && !b.spGim2Done){ b.spGim2Done = true; runSpecialGim(b, ds2); }
      // v6.67 광폭화 (180초 — 150초에 사전 경고): 시간을 끌면 사실상 못 깨는 수준으로 강화 + 체력 10% 회복
      if ((b.aliveT||0) > 150 && !b.frenzyWarn){
        b.frenzyWarn = true;
        showBossBanner('불길한 징조', '"...공기가 무거워진다."', '#d4772e');
        SFX.play('warn');
      }
      if ((b.aliveT||0) > 180 && !b.frenzied){
        b.frenzied = true;
        b.enraged = true;
        b.dmg = Math.round(b.dmg*2.2);
        b.speed *= 1.6;
        b.hp = Math.min(b.maxHp, b.hp + b.maxHp*0.1);
        refreshBossBar();
        showBossBanner('광폭화', '보스가 이성을 잃었다 — 피해 ×2.2 · 속도 ×1.6', '#b8362e');
        effects.push({ type:'ring', x:b.x, y:b.y, life:0.7, age:0, r0:b.r, r1:b.r*4, col:'#b8362e' });
        shake = Math.min(22, shake+12);
        SFX.play('warn');
      }
    }
    ...
    if (!b.finale && (b.aliveT||0) > 55 && !b.midWarned && !b.midWiped){
      b.midWarned = true;
      showBossBanner('경고', '"관문이 요동치기 시작한다..."', '#d4772e');
  ...
  function tickGatePending(dt){
    ...
    clearFieldForGate(); // v6.67 클린 스테이지
  }
  ...
  function updateGateObjs(dt){
    ...
    for (let i=gateObjs.length-1;i>=0;i--){
      ...
      // v6.67 홀드(채널링) 인터랙션: 원 안에 머무르면 게이지가 차오른다 — 이탈하면 서서히 빠짐
      if (o.kind==='hold'){
        const inR = Math.hypot(player.x-o.x, player.y-o.y) < o.r+player.r;
        if (inR){
          o.held = (o.held||0) + dt;
          if (o.held >= (o.holdNeed||1.2)){
            gateObjs.splice(i,1);
            if (o.onTap) o.onTap();
            SFX.play('quest');
            continue;
          ...
          o.held = Math.max(0, (o.held||0) - dt*1.5);
        }
      }
      if (o.t<=0){
        gateObjs.splice(i,1);
  ...
  function drawGateObjs(){
    ...
    for (const o of gateObjs){
      ...
      ctx.strokeStyle = urgent ? '#b8362e' : (o.kind==='qte'||o.kind==='mash' ? '#e2b23f' : o.kind==='hold' ? '#3f7a5c' : '#5a8cc8');
      ctx.lineWidth = 2.2;
      ctx.setLineDash(o.kind==='qte'||o.kind==='mash' ? [5,4] : []);
      ctx.beginPath(); ctx.arc(0,0,o.r*pulse,0,Math.PI*2); ctx.stroke();
      ctx.setLineDash([]);
      // v6.68 리듬: 수축하는 가이드 링 — 코어에 닿는 순간이 정박
      if (o.kind==='rhythm'){
        const gr = o.r + (o.t/o.maxT)*46;
        const inWin = o.t <= (o.window||0.45);
        ctx.strokeStyle = inWin ? '#3f7a5c' : 'rgba(226,178,63,0.7)';
        ctx.lineWidth = inWin ? 3 : 1.6;
        ctx.beginPath(); ctx.arc(0,0,gr,0,Math.PI*2); ctx.stroke();
      }
      // v6.68 진짜 찾기: 진짜에만 흐르는 미세한 금빛 맥동 — 단서는 시각뿐
      if (o.kind==='find' && o.real){
        ctx.globalAlpha = 0.25 + 0.15*Math.sin(performance.now()/210);
        ctx.strokeStyle = '#e8c56a';
        ctx.lineWidth = 1.4;
        ctx.beginPath(); ctx.arc(0,0,o.r*0.6,0,Math.PI*2); ctx.stroke();
        ctx.globalAlpha = 1;
      }
      // v6.67 홀드: 채널링 게이지 (초록 내부 원판이 차오른다)
      if (o.kind==='hold'){
        const hf = Math.min(1, (o.held||0)/(o.holdNeed||1.2));
        ctx.fillStyle = 'rgba(63,122,92,0.35)';
        ctx.beginPath(); ctx.moveTo(0,0); ctx.arc(0,0,o.r*0.85,-Math.PI/2,-Math.PI/2+Math.PI*2*hf); ctx.closePath(); ctx.fill();
      }
      ...
      // v6.67 연타: 남은 횟수 표시
      if (o.kind==='mash'){
        ctx.fillStyle = urgent ? '#b8362e' : '#e2b23f';
        ctx.font = "800 12px 'IBM Plex Sans KR', sans-serif";
        ctx.fillText('×'+(o.need||5), 0, o.r+15);
      }
      // 남은 시간 게이지
  ...
  function updateBoss(b, dt){
    ...
    if (b.kind==='ranged'){
      ...
      if (b.jCallT<=0){
        ...
        addTextNum(player.x, player.y-44, '📞 벨소리가 울린다...');
        b.jCallT = ph===3?7:11;
      }
      ...
      if (b.jPhoneT<=0 && !b.jPhoneOn){
        ...
        addTextNum(player.x, player.y-48, '📱 어디서 많이 본 폰이 떨어져 있다...');
        b.jPhoneT = 16;
      }
      ...
      if (b.gsCycT<=0 && !b.gsShield){
        ...
        addGateObj({ kind:'qte', icon:'⚙', x:b.x+(Math.random()-0.5)*120, y:b.y+(Math.random()-0.5)*120, r:24, maxT:8,
          ...
          onFail:()=>{} });
        showBossBanner('절대 방어', '"뚫리지 않는다."', '#6a7a8a');
        SFX.play('warn');
      }
      if (b.gsShield>0){
        b.gsShield -= dt;
  ...
  function runEngineTicks(dt){
    for (let q=engineTicks.length-1; q>=0; q--){
      const e = engineTicks[q];
      e.t -= dt;
      while (e.t <= 0 && e.i < e.n){ e.fn(e.i); e.i++; e.t += e.iv; }
  ...
  function spendResource(){
    ...
    if (!r || r.v < 1) return;
    if ((player.spendWindT||0) > 0) return;                 // 예비동작 중엔 재입력 무시
    // v6.113 ③ 예비동작 0.25초 — 즉발이 아니다. 위치를 미리 잡아야 하고, 그 사이 피격 위험을 진다
    player.spendWindT = 0.25;
    player.swingT = Math.max(player.swingT||0, 0.3);
  ...
  function spendResourceFire(r){
    ...
    if (near === 0){
      // ② 헛방: 자원만 날아가고 각성 창도 안 열린다
      addTextNum(player.x, player.y-46, '헛방…');
      SFX.play('warn');
      resetSpentResource();
      return;
    }
    ...
    const boom = (R, D, col) => {
      effects.push({ type:'arc', x:player.x, y:player.y, a:0, arc:Math.PI*2, r:R, life:0.26, age:0, friendly:true, col: col||COL });
      friendlyBlast(player.x, player.y, R, D, true);
      cutHostileShots(player.x, player.y, R, 0, Math.PI*2);
    };
    if (key === 'iaido'){
      // 거합 — 유일하게 거리를 만드는 순간. 화면을 가르는 직선 장참 + 확정 치명
      ...
      // 무영난무 — 주변 적 사이를 순간이동하며 차례로 벤다
      const list = enemies.filter(e=>e && Math.hypot(e.x-player.x,e.y-player.y) < 300)
                          .sort((a,b)=>Math.hypot(a.x-player.x,a.y-player.y)-Math.hypot(b.x-player.x,b.y-player.y))
                          .slice(0, 8);
      if (!list.length){ boom(140, 90*P, COL); }
      queueTicks(list.length, 0.075, (i)=>{ if (state!=='playing') return;
        const m = list[i]; if (!m) return;
        particles.push({ x:player.x, y:player.y, vx:0, vy:0, life:0.3, age:0, r:player.r*0.8, ghost:true });
        player.x = m.x - Math.cos(i)*18; player.y = m.y - Math.sin(i)*18;
        effects.push({ type:'slash', x:m.x, y:m.y, a:i*1.7, r:44, arc:2.4, life:0.18, age:0 });
        cutHostileShots(m.x, m.y, 52, 0, Math.PI*2);
        for (let j=enemies.length-1;j>=0;j--){
          const en = enemies[j];
          if (!en || Math.hypot(en.x-m.x, en.y-m.y) > 50 + en.r) continue;
          const d = 74*P*corrodeMult(en);
          en.hp -= d; addDmgNum(en.x, en.y, d, true); staggerEnemy(en, 0.5); procOnHit(en, false, null);
      ...
      addTextNum(player.x, player.y-40, '무영난무');
    } else if (key === 'whirl'){
      // 혈풍륜 — 2초간 멈추지 않고 돈다. 도는 동안 피가 돌아온다
      ...
      queueTicks(CN, 0.07, (i)=>{ if (state!=='playing') return;
        ...
        cutHostileShots(player.x, player.y, 116, a0, last?2.6:1.6);
        for (let j=enemies.length-1;j>=0;j--){
          const e = enemies[j];
          if (!e || Math.hypot(e.x-player.x,e.y-player.y) > 116 + e.r) continue;
          let da = Math.atan2(e.y-player.y, e.x-player.x) - a0;
          while (da>Math.PI) da-=Math.PI*2; while (da<-Math.PI) da+=Math.PI*2;
          if (Math.abs(da) > (last?1.3:0.8)) continue;
          const d = (last?70:24)*P*corrodeMult(e);
          e.hp -= d; addDmgNum(e.x, e.y, d, last); procOnHit(e, false, null); staggerEnemy(e, last?0.7:0.2);
      ...
      for (const e of enemies){
        ...
        e.x += Math.cos(ka)*54; e.y += Math.sin(ka)*54;
        staggerEnemy(e, 1.3);
      }
      shake = Math.min(20, shake+12); hitStop(0.08);
      player.swingT = Math.max(player.swingT||0, 0.42); player.atkMotion = 'heavy';
      addTextNum(player.x, player.y-40, '삼대 오백!');
    } else if (key === 'riposte'){
      // 요지부동 — 1.4초 굳는 대신 무적. 그 사이 다가온 것을 계속 되돌려준다
      player.riposteT = 1.4;
      ...
      player.frenzyFree = 4; player.rushT = 4;
      addTextNum(player.x, player.y-40, '피의 광시곡');
      boom(120, 60*P, '#c9403a');
    } else if (key === 'reap'){
      ...
      for (let i=enemies.length-1;i>=0;i--){
        const e = enemies[i];
        if (!e || e.elite || Math.hypot(e.x-player.x,e.y-player.y) > 196 + e.r) continue;
        if (e.hp>0 && e.hp/Math.max(1,e.maxHp||1) < thr){ e.hp = 0; addTextNum(e.x, e.y-12, '수확'); reaped++; }
        if (e.hp<=0 && enemies[i]===e) defeatEnemy(i);
      }
      shake = Math.min(16, shake+8); hitStop(0.06);
      player.swingT = Math.max(player.swingT||0, 0.4); player.atkMotion = 'drag';
      addTextNum(player.x, player.y-40, reaped ? ('수확의 밤 ×'+reaped) : '수확의 밤');
    } else if (key === 'cadence'){
  ...
  function updateWeapons(dt){
    ...
    for (const w of wlist){
      ...
      if (w.key==='missile'){
      ...
      } else if (MELEE_ENGINE[w.key]){
        // v6.115 근접 평타 — 시그니처(거합·연참·회전·처형…)는 전부 R 소비기로 갔다.
        //  여기 남은 것은 빠르고 약한 '평범한 근접 참격'이고, 본업은 **기력을 쌓는 것**이다.
        const E = MELEE_ENGINE[w.key];
        ...
        for (let i=enemies.length-1;i>=0;i--){
          ...
          const d = dmg*(isC?player.critMult:1)*corrodeMult(e)*crowdMult(hn)*meleeCloseMult(e);
        ...
        let g = E.gain;
        if (w.key==='scythe')       g *= 1 + Math.min(1.0, (hn-1)*0.3);            // 많이 훑을수록
        else if (w.key==='iaido')   g *= (noHitT||0) > 2.5 ? 2.0 : ((noHitT||0) > 1 ? 1.4 : 0.8);  // 안 맞고 벤다
        else if (w.key==='kesagiri') g *= 1 + Math.min(1.2, hn*0.35);              // 한 번에 여럿을 스칠수록
        else if (w.key==='whirl')   g *= 1 + (1-Math.max(0,Math.min(1,player.hp/Math.max(1,player.maxHp))))*1.6;  // 피를 흘릴수록
        else if (w.key==='combo3'){                                                // 끊기지 않고 이어간다
          if ((player.comboT||0) <= 0) player.comboN = 0;
          player.comboT = 1.8; player.comboN = Math.min(12, (player.comboN||0)+1);
          g *= 1 + Math.min(1.4, (player.comboN-1)*0.16);
        }
        else if (w.key==='smash')   g *= 1 + Math.min(1.0, Math.max(0,(player.maxHp-100)/260));   // 몸집이 곧 힘
        else if (w.key==='riposte') g *= (Math.hypot(player.vx||0, player.vy||0) < 18 && player.dashTime<=0) ? 2.1 : 0.4;  // 멈춰 서서
        else if (w.key==='frenzy'){                                                // 제 피를 깎아가며
          g *= 1.9;
  ...
  function update(dt){
    ...
    if (player.dashTime > 0){
      ...
      if (player.riposteT>0){ player.riposteT -= dt; player.invuln = Math.max(player.invuln, 0.1); }      // v6.115 요지부동
      if (player.surgeT>0) player.surgeT -= dt;                                                          // v6.111 각성 창
      if (player.spendWindT>0){                                                                          // v6.113 소비 예비동작
        player.spendWindT -= dt;
        if (player.spendWindT <= 0 && player.spendPend){ const pr = player.spendPend; player.spendPend = null; spendResourceFire(pr); }
      }
    ...
    if (runQuest && (runQuest.type==='noheal' || runQuest.type==='ironman') && player.hp > (player._lastHp===undefined?player.hp:player._lastHp) + 0.01) player.qNoHealHit = true;
    ...
    // v6.68 낙인: 찍혀 있는 동안 발밑이 계속 썩는다 — 멈추면 그대로 맞는다
    if (player.gimMarkT>0){
      player.gimMarkT -= dt;
      player.__markHz = (player.__markHz||0) - dt;
      if (player.__markHz<=0){
        addHazard(player.x, player.y, 48, 0.8, 20*dmgScale(), false);
        player.__markHz = 0.55;
      }
    }
    // v6.68 인력: 보스가 끌어당긴다 — 이동으로 저항하라
    if (player.gimGravT>0){
      player.gimGravT -= dt;
      const gb3 = player.gimGravBoss;
      if (gb3 && bosses.includes(gb3)){
        const gd3 = Math.hypot(gb3.x-player.x, gb3.y-player.y) || 1;
        player.x += (gb3.x-player.x)/gd3 * 135*dt;
        player.y += (gb3.y-player.y)/gd3 * 135*dt;
      } else player.gimGravT = 0;
      if (player.gimGravT<=0) player.gimGravBoss = null;
    }
    // v6.68 정지 명령: 시선이 꽂힌 동안 움직이면 다친다
    if (player.gimStillT>0){
      player.gimStillT -= dt;
      const movingNow = Math.hypot(player.vx||0, player.vy||0) > 25 || player.dashTime>0;
      player.__stillHitT = (player.__stillHitT||0) - dt;
      if (movingNow && player.__stillHitT<=0){
        player.__stillHitT = 0.4;
        addTextNum(player.x, player.y-40, '"움직였군."');
        if (playerHit(10*dmgScale(), 0.15, 6)) return;
      }
    }
    // v6.68 힘겨루기: 시간 내 보스 방향 드래그 필요 (미수행 시 자연 소멸)
    if (player.gimShoveT>0){
      player.gimShoveT -= dt;
      if (player.gimShoveT<=0) player.gimShoveBoss = null;
    }
    // v6.67 시한폭탄 기믹: 보스 곁에서 터뜨리면 되돌려주고, 멀리서 터지면 내가 다친다
    if (player.gimBombT>0){
      player.gimBombT -= dt;
      if (player.gimBombT<=0){
        const bb = player.gimBombBoss;
        const nearB = bb && bosses.includes(bb) && Math.hypot(player.x-bb.x, player.y-bb.y) < 170;
        effects.push({ type:'ring', x:player.x, y:player.y, life:0.5, age:0, r0:20, r1:170, col:'#b8362e' });
        burst(player.x, player.y, 14, 200, 0xe2603f, COLORS.boom);
        shake = Math.min(18, shake+10);
        SFX.play('boom');
        player.gimBombBoss = null;
        if (nearB){
          bb.hp -= bb.maxHp*0.08;
          addDmgNum(bb.x, bb.y, bb.maxHp*0.08, true);
          addTextNum(bb.x, bb.y-bb.r-16, '되돌려줬다!');
          refreshBossBar();
          if (bb.hp<=0){ const bi=bosses.indexOf(bb); if (bi>=0) defeatBoss(bi); }
        } else {
          addTextNum(player.x, player.y-40, '너무 멀리서 터졌다...');
          if (playerHit(30*dmgScale(), 0.5, 12)) return;
        }
      }
    }
    ...
    // v6.67 관문 집중 모드: 관문 전투 중엔 계약·조사·제단·균열·의뢰인·상인이 일절 등장하지 않는다
    const gateBattle = bosses.some(b2=>b2.gate);
    // 중간 계약 (4분 / 8분)
    if (!gateBattle && midContractIdx < midContractTimes.length && elapsed >= midContractTimes[midContractIdx]){
      midContractIdx += 1;
      openMidContract();
      return;
    }
    // 등장 주기 랜덤화: 매판·매회 다른 타이밍에 나타난다 (관문 중엔 타이머만 미루고 스폰 안 함)
    if (elapsed >= nextSurveyAt){ nextSurveyAt = elapsed + SURVEY_INTERVAL*(0.6+Math.random()*0.9); if (!gateBattle) spawnSurvey(); }
    if (elapsed >= nextAltarAt){ nextAltarAt = elapsed + 75*(0.7+Math.random()*0.8); if (!gateBattle) spawnAltar(); }
    if (elapsed >= nextRiftAt){ nextRiftAt = elapsed + 140*(0.7+Math.random()*0.7); if (!gateBattle) spawnRift(); }
    // 차원 균열 시련 진행
    if (rift){
      rift.t -= dt;
      let done = false;
    ...
    if (!gateBattle && elapsed >= 45 + clientCount*90 && clients.length===0 && !runQuest){
      clientCount += 1;
    ...
    if (!gateBattle && elapsed >= 90 + merchantCount*120*((player&&player.merchantFreq)||1) && merchants.length===0){
      merchantCount += 1;
    ...
    if (trialT > 0){
      ...
      if (trialT <= 0){
        ...
        questAdd('trial', 1);
        // 난이도별 보상: 쉬움(1성) < 보통(2성) < 지옥(3성) — 힘든 시련일수록 별이 쏟아진다
        const TSTAR = { horde:1, gold:1, gravity:1, haste:2, elite:2, bombard:2, dark:3, silence:3, bleed:3, frenzy:3 };
        const star = TSTAR[trialKind]||1;
        dropItem(player.x+40, player.y-30, 'chest');
        if (star>=2) dropItem(player.x-40, player.y-30, trialKind==='elite'?'whet':'chest');
        if (star>=3){ dropItem(player.x, player.y-60, 'scroll'); DB.mats.shard += 1; }
        const g = gainGold(40*star);
        addTextNum(player.x, player.y-30, '시련 완수! ('+'★'.repeat(star)+') +'+g+'G');
        effects.push({ type:'rays', x:player.x, y:player.y, life:0.7, age:0 });
        toast('시련 완수 '+'★'.repeat(star)+' — 보상 '+star+'배급'+(star>=3?' + ★조각':''));
        SFX.play('win');
      }
    }
        ...
        for (const sp of satPos){
          ...
          if (sd < e.r+(sp.ev?13:10)){
            if (!(e.satCd>0)){
              ...
              e.hp -= sdmg;
              //  v6.143 ① 재타격 간격: 0.28×1.35(버그로 읽히던 값) ≈ **0.38초를 그대로 유지**한다.
              //   0.50으로 늘려 봤지만 룬 기사 3회 평균이 102.3 → 102.3으로 **전혀 안 움직였다**.
              //   진단해 보니 이유가 명확했다 — 룬 기사 12초 킬: 정상 114 / 위성 화력 0으로 **109(−4%)** /
              //   스킬·궁극·R 없이 **11(−90%)**. **위성은 킬의 4%다.** 근거 없는 너프는 넣지 않는다
              //  🐛 ② `player.satOrbit||1` → `||0`. `0||1`은 1이라 **표준 궤도인데 확장 궤도의 cd를 읽고** 있었다.
              //   위치·피해는 `||0`으로 읽는데 여기만 달랐다 — 같은 상태를 두 군데서 다르게 읽고 있었다.
              //   ⚠ 인덱스 기본값에 `||`를 쓰지 말 것. **0을 넣는 순간 뜻이 갈린다**
              e.satCd = 0.38 * (SAT_ORBIT[player.satOrbit || 0].cd);
              satCharge(0.035);
  ...
  function playerHit(dmg, invulnTime, shakeAmt){
    ...
    noHitT = 0;
    // 생존 의뢰 실패
    player.qHitFlag = true;
    if (runQuest && (runQuest.type==='nohit' || runQuest.type==='ironman')){
      toast('의뢰 실패... (피격)');
      runQuest = null;
    }
  ...
  function renderDex(){
    ...
    // 직업 성장무기 도감 (37직업 × 3 = 111종): 전체 목록 상시 공개 — 이름은 보이고, 발견하면 금색+레벨
    {
      let cgwFoundN = 0, cgwTotal = 0;
      const lines = [];
      Object.keys(CGW_NAMES).forEach(ck=>{
        const cn = CLASSES[ck] ? CLASSES[ck].name : ck;
        const isMine = DB.lastClass===ck;
        const parts = CGW_NAMES[ck].map((nm,i)=>{
          ...
          return '<span style="opacity:0.55;">'+nm+'</span>';
        });
  ...
  function openClientQuest(){
    ...
    const offers = [
      ...
      // 고난이도 의뢰 — 보상도 그만큼 (상자 2개 = chest2)
      { l:'🔥 학살극', d:'60초 안에 120마리 → 350G + 상자 2개', q:{ type:'kill', goal:120, t:60, start:killCount, gold:350, chest:false, chest2:true } },
      { l:'🔥 무결점 계약', d:'60초 무피격 → 400G + 상자 2개', q:{ type:'nohit', goal:0, t:60, gold:400, chest:false, chest2:true } },
      { l:'🔥 광란의 콤보', d:'콤보 80 달성 (75초) → 450G + 상자 2개', q:{ type:'combo', goal:80, t:75, gold:450, chest:false, chest2:true } },
      { l:'🔥 큰손의 시험', d:'60초 안에 골드 200 벌기 → 500G', q:{ type:'gold', goal:200, t:60, gold:500, chest:true } },
      { l:'🔥 고행자의 길', d:'75초간 회복 금지 + 무피격 → 600G + 상자 2개', q:{ type:'ironman', goal:0, t:75, gold:600, chest:false, chest2:true } },
    ];
    const pick2 = [];
    ...
    const opts = pick2.map(o=>({ l:o.l, d:o.d, fx:()=>{
      ...
      if (o.q.type==='noheal' || o.q.type==='ironman'){ player.qNoHealHit = false; player.qHitFlag = false; }
      toast('의뢰 수락: '+o.l);
      SFX.play('quest');
    } }));
  ...
  function tickRunQuest(dt){
    ...
    } else if (runQuest.type==='ironman'){
      if (player.qNoHealHit || player.qHitFlag){ fail = true; } else if (runQuest.t<=0) done = true;
    ...
    if (done){
      ...
      if (runQuest.chest2){ dropItem(player.x+40, player.y, 'chest'); dropItem(player.x-40, player.y, 'chest'); }
      toast('의뢰 완수!');
      effects.push({ type:'rays', x:player.x, y:player.y, life:0.6, age:0 });
      SFX.play('win');
      runQuest = null;
  ...
  function startGame(classKey){
    ...
    if (cls){
      ...
      if (cls.weapon==='random2' || cls.weapon==='random3'){
        // 무작위 무기 (기본 무기 풀에서)
        const keys = Object.keys(WEAPONS).filter(k=>!['fusion','gbow','gtome','gblade','nameless'].includes(k) && k.indexOf('cgw_')!==0 && k.indexOf('pgw_')!==0);
    ...
    if (gwSel && gwFound[gwSel] && !ownedWeapon(gwSel)){
      if (GWEP_OWNER[gwSel]===classKey){
        ...
        toast('⚔ ['+WEAPONS[gwSel].name+'] — ['+(CLASSES[GWEP_OWNER[gwSel]]?CLASSES[GWEP_OWNER[gwSel]].name:'')+'] 전용이다 (미장착 출전)');
      }
    ...
    } else if (gwSel && gwSel.indexOf('pgw_')===0){
      const prec2 = (DB.pgw||{})[gwSel.slice(4)];
      if (prec2 && prec2.found && gwSel.indexOf('pgw_'+classKey+'_')===0 && !ownedWeapon(gwSel) && WEAPONS[gwSel]){
        addWeapon(gwSel);
        toast('직업 성장무기와 함께 출전: '+WEAPONS[gwSel].name);
      } else if (prec2 && prec2.found){
        toast('⚔ 직업 성장무기는 그 직업만 들 수 있다 — 미장착 출전');
      }
    }
  ...
  function bankRun(){
    ...
    // 직업 성장무기: 이 런에서 함께 싸운 만큼 자란다 (처치 수 흡수)
    if (player && player.weapons){
      const pw2 = player.weapons.find(w2=>w2.key && w2.key.indexOf('pgw_')===0);
      if (pw2){
        const rec2 = pgwRec(pw2.key.slice(4));
        rec2.xp = (rec2.xp||0) + killCount;
        let walled = false;
        while (rec2.lv < 50 && rec2.xp >= 140 + rec2.lv*85){ // 만렙 50 — 긴 여정 끝의 왕귀
          const need = {10:1,20:2,30:3,40:4}[rec2.lv];
  ...
  function drawHumanoidVec(x, y, o){
    ...
    if (atkN > 0){
      ...
      } else if (o.motion === 'sheath'){
        // 발도 평타: 짧게 낮추었다가(칼집에 손) 뽑아 베고 즉시 납도 — 되돌림이 빠르다
        if (t < 0.3){ const k=t/0.3; swRot = 0.5*k; lunge = -1.4*k; lunY = 0.9*k; }
        else if (t < 0.55){ const k=(t-0.3)/0.25; const e=1-Math.pow(1-k,3);
  ...
  function drawPlayerChar(){
    ...
    // v6.68 힘겨루기 마커: 보스 쪽으로 화면을 그어라
    if (player.gimShoveT>0){
      ctx.save();
      ctx.fillStyle = '#e2823f';
      ctx.font = "800 12px 'IBM Plex Sans KR', sans-serif";
      ctx.textAlign = 'center';
      ctx.fillText('✋⇢ '+player.gimShoveT.toFixed(1), player.x, player.y-58);
      ctx.restore();
    }
    // v6.67 시한폭탄 카운트다운 마커
    if (player.gimBombT>0){
  ...
  function drawBoss(b){
    ...
    if (!b.ghost){
      ...
      if (b._pipPrev!==undefined && pipN < b._pipPrev){
        ...
        shake = Math.min(14, shake+6);
        // v6.68 페이즈 전환 연출 컷: 짧은 정지 + 암전 — "국면이 바뀐다"
        freeze = Math.max(freeze, 0.5);
        screenDimT = Math.max(screenDimT, 0.6);
      }
      b._pipPrev = pipN;
      // 빈사(25% 미만): 붉은 균열 파편
      if (bfrac < 0.25){
    ...
    // v6.68 오라 정리: 몸 주위 이중 링 제거 — 바닥 그라운드 링(후단)과 분노 펄스만 남겨 위엄을 살린다
    ...
    // v6.68 거리 심판: 붉은 위험 반경 — 판정 순간 안에 있으면 벌
    if (b.spDistT>0 && !b.ghost){
      ctx.save();
      ctx.strokeStyle = 'rgba(184,54,46,'+(0.35+0.25*Math.sin(performance.now()/90)).toFixed(2)+')';
      ctx.lineWidth = 2.5;
      ctx.beginPath(); ctx.arc(b.x, b.y, 240, 0, Math.PI*2); ctx.stroke();
      ctx.restore();
    }
    // v6.68 응시: 점선 조준선 — 사라지기 전에 측면으로
    if (b.spGazeT>0 && !b.ghost){
      ctx.save();
      ctx.strokeStyle = 'rgba(154,111,196,0.65)';
      ctx.lineWidth = 2;
      ctx.setLineDash([8,6]);
      ctx.beginPath(); ctx.moveTo(b.x, b.y); ctx.lineTo(b.x+Math.cos(b.spGazeA)*900, b.y+Math.sin(b.spGazeA)*900); ctx.stroke();
      ctx.setLineDash([]);
      ctx.restore();
    }
  ...
  function drawDmgNums(){
    ...
    for (const n of dmgNums){
      ...
      if (n.label){
        // v6.68 가독성: 배경·오라 위에서도 읽히게 헤일로 스트로크
        ctx.font = "700 12px 'IBM Plex Sans KR', sans-serif";
        ctx.strokeStyle = MAP.key==='abyss' ? 'rgba(18,19,22,0.85)' : 'rgba(246,246,244,0.9)';
        ctx.lineWidth = 3;
    ...
    // v6.107 엔진 자원 게이지 — 캐릭터 발밑. 만충이면 맥동하며 'R' 안내가 뜬다
    {
      ...
      if (rs){
        ...
        if (full){                            // v6.113 지금 쓰면 몇 마리가 걸리는지 — 판단 근거를 보여준다
          ...
          window.__spendHint = n2;
          ctx.globalAlpha = 0.85;
          ctx.fillStyle = n2 >= 3 ? '#e0a94f' : n2 === 0 ? '#c9403a' : rs.c;
          ctx.font = "700 8px 'IBM Plex Mono', monospace";
          ctx.fillText(n2 === 0 ? '헛방' : ('x'+n2), player.x + 26, by + 4);
        }
